...
class DemoFinalExternalReviewReleaseBundleArchiveMigrationTests {

    @Test
    void creates_final_external_review_release_bundle_archive_table() throws Exception {
        String migration = Files.readString(Path.of(
                "src/main/resources/db/migration/V58__create_demo_final_external_review_release_bundle_archive.sql"
        ));

        assertThat(migration)
                .contains("CREATE TABLE IF NOT EXISTS demo_final_external_review_release_bundle_archive");
        assertThat(migration).contains("id VARCHAR(64) PRIMARY KEY");
        assertThat(migration).contains("status VARCHAR(32) NOT NULL");
        assertThat(migration).contains("release_ready BOOLEAN NOT NULL");
        ...
        assertThat(migration).contains("release_checks_json JSON NOT NULL");
        ...
        assertThat(migration).contains("idx_final_external_review_release_bundle_archive_archived_at");
        assertThat(migration).contains("idx_final_external_review_release_bundle_archive_status");
        assertThat(migration).contains("idx_final_external_review_release_bundle_archive_ready");
    }
}
