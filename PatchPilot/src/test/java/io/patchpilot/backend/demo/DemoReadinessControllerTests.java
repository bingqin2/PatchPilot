...
class DemoReadinessControllerTests {
    ...
    @MockitoBean
    private DemoFinalExternalReviewEvidencePackageArchiveService demoFinalExternalReviewEvidencePackageArchiveService;
    ...
    @Test
    void should_archive_final_external_review_evidence_package_and_record_audit() throws Exception {
        when(demoFinalExternalReviewEvidencePackageArchiveService.archiveCurrentPackage())
                .thenReturn(finalExternalReviewEvidencePackageArchive());

        mockMvc.perform(post("/api/demo/final-external-review-evidence-package/archives"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.id").value("final-external-review-package-archive-1"))
                .andExpect(jsonPath("$.data.status").value("READY"))
                .andExpect(jsonPath("$.data.readyForExternalReview").value(true))
                .andExpect(jsonPath("$.data.latestTaskId").value("task-1"))
                .andExpect(jsonPath("$.data.latestPullRequestUrl").value("https://github.com/bingqin2/PatchPilot/pull/8"))
                ...
                .andExpect(jsonPath("$.data.completionEvidenceDeliveryReceiptId")
                        .value("final-acceptance-completion-evidence-delivery-receipt-1"))
                .andExpect(jsonPath("$.data.closeoutArchiveId").value("final-acceptance-completion-closeout-archive-1"))
                .andExpect(jsonPath("$.data.deliveryReceiptFreshness").value("FRESH"))
        ...
        verify(operatorSafetyAuditService).recordSafetyAudit(argThat(command ->
                command.action().equals("DEMO_FINAL_EXTERNAL_REVIEW_PACKAGE_ARCHIVED")
                        && command.resourceType().equals("DEMO_FINAL_EXTERNAL_REVIEW_PACKAGE_ARCHIVE")
                        && command.resourceId().equals("final-external-review-package-archive-1")
                        && command.reason().contains("final-acceptance-completion-closeout-archive-1")
        ));
    }

    @Test
    void should_reject_final_external_review_evidence_package_archive_when_package_is_not_ready() throws Exception {
        when(demoFinalExternalReviewEvidencePackageArchiveService.archiveCurrentPackage())
                .thenThrow(new IllegalStateException("final external-review evidence package is not ready"));

        mockMvc.perform(post("/api/demo/final-external-review-evidence-package/archives"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("final external-review evidence package is not ready"));
    }

    @Test
    void should_list_final_external_review_evidence_package_archives() throws Exception {
        when(demoFinalExternalReviewEvidencePackageArchiveService.listRecentArchives())
                .thenReturn(List.of(finalExternalReviewEvidencePackageArchive()));

        mockMvc.perform(get("/api/demo/final-external-review-evidence-package/archives"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.length()").value(1))
                .andExpect(jsonPath("$.data[0].id").value("final-external-review-package-archive-1"))
                .andExpect(jsonPath("$.data[0].archivedAt").value("2026-06-29T08:30:00Z"));
    }

    @Test
    void should_download_archived_final_external_review_evidence_package_report() throws Exception {
        when(demoFinalExternalReviewEvidencePackageArchiveService.findArchive(
                "final-external-review-package-archive-1"
        )).thenReturn(Optional.of(finalExternalReviewEvidencePackageArchive()));

        mockMvc.perform(get("/api/demo/final-external-review-evidence-package/archives/final-external-review-package-archive-1/report/download"))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString(
                        "patchpilot-final-external-review-evidence-package-final-external-review-package-archive-1.md"
                )))
                .andExpect(content().contentType("text/markdown;charset=UTF-8"))
                .andExpect(content().string(containsString(
                        "# PatchPilot Final External Review Evidence Package"
    ...
    @Test
    void should_return_not_found_when_final_external_review_evidence_package_archive_is_missing() throws Exception {
        when(demoFinalExternalReviewEvidencePackageArchiveService.findArchive("missing"))
    ...
    @Test
    void should_return_final_external_review_delivery_certificate() throws Exception {
        when(demoFinalExternalReviewDeliveryCertificateService.getCertificate())
                .thenReturn(finalExternalReviewDeliveryCertificate());

        mockMvc.perform(get("/api/demo/final-external-review-delivery-certificate"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.status").value("READY"))
                .andExpect(jsonPath("$.data.certified").value(true))
                .andExpect(jsonPath("$.data.summary")
                        .value("Final external-review delivery is certified from the latest finalized archive."))
    ...
    @Test
    void should_download_final_external_review_delivery_certificate_report() throws Exception {
        when(demoFinalExternalReviewDeliveryCertificateService.getCertificate())
                .thenReturn(finalExternalReviewDeliveryCertificate());

        mockMvc.perform(get("/api/demo/final-external-review-delivery-certificate/report/download"))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString(
    ...
    @Test
    void should_return_final_external_review_release_bundle() throws Exception {
        when(demoFinalExternalReviewReleaseBundleService.getReleaseBundle())
                .thenReturn(finalExternalReviewReleaseBundle());

        mockMvc.perform(get("/api/demo/final-external-review-release-bundle"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.status").value("READY"))
                .andExpect(jsonPath("$.data.releaseReady").value(true))
                .andExpect(jsonPath("$.data.summary")
                        .value("PatchPilot final external-review release bundle is ready."))
                .andExpect(jsonPath("$.data.latestCertificateArchiveId")
                        .value("final-external-review-delivery-certificate-archive-1"))
                .andExpect(jsonPath("$.data.latestDeliveryReceiptId")
                        .value("final-external-review-package-delivery-receipt-1"))
                .andExpect(jsonPath("$.data.requiredAttachments").value(hasItem(
    ...
    @Test
    void should_download_final_external_review_release_bundle_report() throws Exception {
        when(demoFinalExternalReviewReleaseBundleService.getReleaseBundle())
                .thenReturn(finalExternalReviewReleaseBundle());

        mockMvc.perform(get("/api/demo/final-external-review-release-bundle/report/download"))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString(
    ...
    @Test
    void should_archive_final_external_review_release_bundle_delivery_certificate_and_record_audit()
            throws Exception {
        when(demoFinalExternalReviewReleaseBundleDeliveryCertificateArchiveService.archiveCurrentCertificate())
                .thenReturn(finalExternalReviewReleaseBundleDeliveryCertificateArchive());

        mockMvc.perform(post("/api/demo/final-external-review-release-bundle/delivery-certificate/archives"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.id")
        ...
        verify(operatorSafetyAuditService).recordSafetyAudit(argThat(command ->
                command.action().equals("DEMO_FINAL_EXTERNAL_REVIEW_RELEASE_BUNDLE_DELIVERY_CERTIFICATE_ARCHIVED")
                        && command.resourceType()
                        .equals("DEMO_FINAL_EXTERNAL_REVIEW_RELEASE_BUNDLE_DELIVERY_CERTIFICATE_ARCHIVE")
                        && command.resourceId()
                        .equals("final-external-review-release-bundle-delivery-certificate-archive-1")
                        && command.reason().contains("final-external-review-release-bundle-delivery-receipt-1")
        ));
    }

    @Test
    void should_reject_final_external_review_release_bundle_delivery_certificate_archive_when_not_certified()
            throws Exception {
        when(demoFinalExternalReviewReleaseBundleDeliveryCertificateArchiveService.archiveCurrentCertificate())
                .thenThrow(new IllegalStateException(
        ...
        mockMvc.perform(post("/api/demo/final-external-review-release-bundle/delivery-certificate/archives"))
                ...
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message")
                        .value("final external-review release bundle delivery certificate is not certified"));
    }

    @Test
    void should_list_final_external_review_release_bundle_delivery_certificate_archives() throws Exception {
        when(demoFinalExternalReviewReleaseBundleDeliveryCertificateArchiveService.listRecentArchives())
                .thenReturn(List.of(finalExternalReviewReleaseBundleDeliveryCertificateArchive()));

        mockMvc.perform(get("/api/demo/final-external-review-release-bundle/delivery-certificate/archives"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.length()").value(1))
                .andExpect(jsonPath("$.data[0].id")
                ...
                .andExpect(jsonPath("$.data[0].latestDeliveryReceiptId")
                        .value("final-external-review-release-bundle-delivery-receipt-1"))
                .andExpect(jsonPath("$.data[0].archivedAt").value("2026-06-29T15:30:00Z"));
    }

    @Test
    void should_download_archived_final_external_review_release_bundle_delivery_certificate_report()
            throws Exception {
        when(demoFinalExternalReviewReleaseBundleDeliveryCertificateArchiveService.findArchive(
                "final-external-review-release-bundle-delivery-certificate-archive-1"
        )).thenReturn(Optional.of(finalExternalReviewReleaseBundleDeliveryCertificateArchive()));

        mockMvc.perform(get("/api/demo/final-external-review-release-bundle/delivery-certificate/archives/final-external-review-release-bundle-delivery-certificate-archive-1/report/download"))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString(
                        "patchpilot-final-external-review-release-bundle-delivery-certificate-final-external-review-release-bundle-delivery-certificate-archive-1.md"
                )))
                .andExpect(content().contentType("text/markdown;charset=UTF-8"))
                .andExpect(content().string(containsString(
                        "# PatchPilot Final External Review Release Bundle Delivery Certificate"
    ...
    @Test
    void should_return_not_found_when_final_external_review_release_bundle_delivery_certificate_archive_is_missing()
            throws Exception {
        when(demoFinalExternalReviewReleaseBundleDeliveryCertificateArchiveService.findArchive("missing"))
                .thenReturn(Optional.empty());
    ...
    @Test
    void should_record_final_external_review_package_delivery_receipt_and_record_audit() throws Exception {
        when(demoFinalExternalReviewEvidencePackageDeliveryReceiptService.recordDeliveryReceipt(argThat(request ->
                request.deliveryChannel().equals("email")
                        && request.deliveryTarget().equals("reviewer@example.com")
                        && request.operator().equals("local-operator")
                        && request.notes().equals("Sent frozen final external-review package to the reviewer.")
                        && request.deliveredAt().equals(Instant.parse("2026-06-29T09:25:00Z"))
        ))).thenReturn(finalExternalReviewEvidencePackageDeliveryReceipt());

        mockMvc.perform(post("/api/demo/final-external-review-evidence-package/delivery-receipts")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "deliveryChannel": "email",
                                  "deliveryTarget": "reviewer@example.com",
                                  "operator": "local-operator",
                                  "notes": "Sent frozen final external-review package to the reviewer.",
                                  "deliveredAt": "2026-06-29T09:25:00Z"
                                }
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.id").value("final-external-review-package-delivery-receipt-1"))
                .andExpect(jsonPath("$.data.status").value("READY"))
                .andExpect(jsonPath("$.data.finalExternalReviewPackageArchiveStatus").value("READY"))
                .andExpect(jsonPath("$.data.finalExternalReviewPackageArchiveId")
                        .value("final-external-review-package-archive-1"))
                .andExpect(jsonPath("$.data.closeoutArchiveId")
                        .value("final-acceptance-completion-closeout-archive-1"))
                .andExpect(jsonPath("$.data.deliveryTarget").value("reviewer@example.com"))
        ...
        verify(operatorSafetyAuditService).recordSafetyAudit(argThat(command ->
                command.action().equals("DEMO_FINAL_EXTERNAL_REVIEW_PACKAGE_DELIVERY_RECEIPT_RECORDED")
                        && command.resourceType().equals("DEMO_FINAL_EXTERNAL_REVIEW_PACKAGE_DELIVERY_RECEIPT")
                        && command.resourceId().equals("final-external-review-package-delivery-receipt-1")
                        && command.operator().equals("local-operator")
                        && command.reason().contains("final-external-review-package-archive-1")
        ));
    }

    @Test
    void should_reject_final_external_review_package_delivery_receipt_when_archive_is_not_ready() throws Exception {
        when(demoFinalExternalReviewEvidencePackageDeliveryReceiptService.recordDeliveryReceipt(
                any(DemoFinalExternalReviewEvidencePackageDeliveryReceiptRequestDto.class)
        )).thenThrow(new IllegalStateException("final external-review evidence package archive is not ready for delivery"));
        ...
        mockMvc.perform(post("/api/demo/final-external-review-evidence-package/delivery-receipts")
                ...
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message")
                        .value("final external-review evidence package archive is not ready for delivery"));
    }

    @Test
    void should_list_final_external_review_package_delivery_receipts() throws Exception {
        when(demoFinalExternalReviewEvidencePackageDeliveryReceiptService.listRecentReceipts())
                .thenReturn(List.of(finalExternalReviewEvidencePackageDeliveryReceipt()));

        mockMvc.perform(get("/api/demo/final-external-review-evidence-package/delivery-receipts"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.length()").value(1))
                .andExpect(jsonPath("$.data[0].id").value("final-external-review-package-delivery-receipt-1"))
                .andExpect(jsonPath("$.data[0].finalExternalReviewPackageArchiveId")
                        .value("final-external-review-package-archive-1"));
    }

    @Test
    void should_download_final_external_review_package_delivery_receipt_report() throws Exception {
        when(demoFinalExternalReviewEvidencePackageDeliveryReceiptService.findReceipt(
                "final-external-review-package-delivery-receipt-1"
        )).thenReturn(Optional.of(finalExternalReviewEvidencePackageDeliveryReceipt()));

        mockMvc.perform(get("/api/demo/final-external-review-evidence-package/delivery-receipts/final-external-review-package-delivery-receipt-1/report/download"))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString(
                        "patchpilot-final-external-review-package-delivery-receipt-final-external-review-package-delivery-receipt-1.md"
                )))
                .andExpect(content().contentType("text/markdown;charset=UTF-8"))
                .andExpect(content().string(containsString(
                        "# PatchPilot Final External Review Package Delivery Receipt"
    ...
    @Test
    void should_return_not_found_when_final_external_review_package_delivery_receipt_is_missing() throws Exception {
        when(demoFinalExternalReviewEvidencePackageDeliveryReceiptService.findReceipt("missing-receipt"))
                .thenReturn(Optional.empty());
    ...
    private static DemoFinalExternalReviewDeliveryCertificateVo finalExternalReviewDeliveryCertificate() {
    ...
    private static DemoFinalExternalReviewReleaseBundleDeliveryCertificateArchiveVo
    finalExternalReviewReleaseBundleDeliveryCertificateArchive() {
