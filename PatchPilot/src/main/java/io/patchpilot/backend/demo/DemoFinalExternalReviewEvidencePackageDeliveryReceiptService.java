...
@Service
public class DemoFinalExternalReviewEvidencePackageDeliveryReceiptService {
    ...
    private final DemoFinalExternalReviewEvidencePackageDeliveryReceiptRepository receiptRepository;
    private final Clock clock;
    private final Supplier<String> idSupplier;

    @Autowired
    public DemoFinalExternalReviewEvidencePackageDeliveryReceiptService(
    ...
    public DemoFinalExternalReviewEvidencePackageDeliveryReceiptVo recordDeliveryReceipt(
            DemoFinalExternalReviewEvidencePackageDeliveryReceiptRequestDto request
    ) {
        DemoFinalExternalReviewEvidencePackageArchiveVo archive = archiveSupplier.get().stream()
                .findFirst()
                .orElseThrow(() -> new IllegalStateException(
                        "no final external-review evidence package archive is available"
                ));
        if (archive.status() != DemoReadinessStatus.READY || !archive.readyForExternalReview()) {
            throw new IllegalStateException(
                    "final external-review evidence package archive is not ready for delivery"
            );
        }
        ...
        return receiptRepository.save(new DemoFinalExternalReviewEvidencePackageDeliveryReceiptVo(
                idSupplier.get(),
                DemoReadinessStatus.READY,
                archive.status(),
                archive.id(),
                valueOrNone(archive.closeoutArchiveId()),
                valueOrNone(archive.completionArchiveId()),
                valueOrNone(archive.completionEvidenceDeliveryReceiptId()),
    ...
    public List<DemoFinalExternalReviewEvidencePackageDeliveryReceiptVo> listRecentReceipts() {
        return receiptRepository.listRecentReceipts(MAX_RECEIPTS);
    }

    public Optional<DemoFinalExternalReviewEvidencePackageDeliveryReceiptVo> findReceipt(String receiptId) {
    ...
    private static String formatMarkdownReport(
        ...
        builder.append("# PatchPilot Final External Review Package Delivery Receipt\n\n");
        builder.append("- Status: `READY`\n");
        builder.append("- Final external-review package archive status: `").append(archive.status()).append("`\n");
        builder.append("- Final external-review package archive: `").append(archive.id()).append("`\n");
        builder.append("- Closeout archive: `").append(valueOrNone(archive.closeoutArchiveId())).append("`\n");
        builder.append("- Completion archive: `").append(valueOrNone(archive.completionArchiveId())).append("`\n");
        builder.append("- Completion evidence delivery receipt: `")
                .append(valueOrNone(archive.completionEvidenceDeliveryReceiptId()))
                .append("`\n");
        ...
        appendList(builder, "Frozen Package Evidence Notes", archive.evidenceNotes());
        appendList(builder, "Frozen Package Download Actions", archive.downloadActions());
        builder.append("## Notes\n\n");
        builder.append(notes.isBlank() ? "No delivery notes recorded." : notes).append("\n\n");
        builder.append("## Source Final External-Review Package Archive\n\n");
        builder.append(archive.report()).append("\n\n");
        builder.append("## Side-Effect Contract\n\n");
        builder.append("POST /api/demo/final-external-review-evidence-package/delivery-receipts ");
