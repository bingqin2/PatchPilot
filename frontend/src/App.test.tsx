...
const githubPublishPermissionReadiness = {
  status: 'READY',
...
const externalExposureReadiness = {
  status: 'NEEDS_ATTENTION',
  safeToExpose: false,
  readyCount: 8,
  needsAttentionCount: 2,
  ...
  checks: [
    {
      ...
      status: 'READY',
      summary: 'Admin API token is configured.',
      nextAction: 'No action needed.'
    },
    {
      name: 'Trigger user allowlist',
      status: 'NEEDS_ATTENTION',
      summary: 'Trigger user allowlist is open.',
      nextAction: 'Configure PATCHPILOT_ALLOWED_TRIGGER_USERS before inviting external trigger traffic.'
    }
  ],
  generatedAt: '2026-07-01T12:00:00Z',
...
const demoFinalExternalReviewReleaseBundleDeliveryReceipt = {
  ...
  nextAction: 'Use the delivery receipt as proof that the frozen final release bundle was shared.',
  deliveryChannel: 'email',
  deliveryTarget: 'external-review@example.com',
  operator: 'release-captain',
  notes: 'Sent final release bundle with all listed attachments.',
  deliveredAt: '2026-06-29T10:45:00Z',
  createdAt: '2026-06-29T10:50:00Z',
  markdownReport: '# PatchPilot Final External Review Release Bundle Delivery Receipt'
};

const demoFinalExternalReviewReleaseBundleDeliveryFinalization = {
  status: 'READY',
  finalized: true,
  summary: 'Final external-review release bundle delivery is finalized with a fresh receipt.',
  nextAction: 'Use the finalization report as the reviewer-facing release delivery record.',
  ...
  latestTaskId: 'task-1',
  latestPullRequestUrl: 'https://github.com/bingqin2/PatchPilot/pull/8',
  latestDeliveryTarget: 'external-review@example.com',
  latestDeliveryChannel: 'email',
  latestDeliveredAt: '2026-06-29T10:45:00Z',
  ...
  checks: [
    {
      name: 'Frozen release bundle archive',
      status: 'READY',
      summary: 'Frozen release bundle archive final-external-review-release-bundle-archive-1 has a fresh delivery receipt.',
      nextAction: 'No action needed.'
    }
  ],
  evidenceNotes: [
    'Frozen release bundle archive final-external-review-release-bundle-archive-1 was delivered.'
  ],
  downloadActions: [
  ...
  sideEffectContract:
    'GET /api/demo/final-external-review-release-bundle/delivery-finalization is read-only: it does not create tasks, call the model, run tests, archive records, record receipts, mutate Git, send messages, or write to GitHub.',
...
const demoFinalExternalReviewReleaseBundleDeliveryCertificate = {
  status: 'READY',
  certified: true,
  summary:
  ...
  sideEffectContract:
    'GET /api/demo/final-external-review-release-bundle/delivery-certificate is read-only: it does not create tasks, call the model, run tests, archive records, record receipts, mutate Git, send messages, or write to GitHub.',
  markdownReport:
    '# PatchPilot Final External Review Release Bundle Delivery Certificate',
  generatedAt: '2026-06-29T11:20:00Z'
};
...
const demoEvidenceBundle = {
  ...
  finalExternalReviewReleaseBundleArchiveEvidence: {
    ...
    archived: true,
    releaseReady: true,
    summary: 'Latest final external-review release bundle archive is frozen and ready.',
    nextAction: 'Use the archived final external-review release bundle as the reviewer-facing source.',
    archiveCount: 1,
    latestArchiveId: 'final-external-review-release-bundle-archive-1',
    latestCertificateArchiveId: 'final-external-review-delivery-certificate-archive-1',
    latestDeliveryFinalizationArchiveId: 'final-external-review-package-delivery-finalization-archive-1',
    ...
    latestTaskId: 'task-1',
    latestPullRequestUrl: 'https://github.com/bingqin2/PatchPilot/pull/8',
    latestArchivedAt: '2026-06-29T10:35:00Z',
    downloadActions: [
      'Download final external-review release bundle archive final-external-review-release-bundle-archive-1.',
  ...
  finalExternalReviewReleaseBundleDeliveryCertificateArchiveEvidence: {
    ...
    latestTaskId: 'task-1',
    latestPullRequestUrl: 'https://github.com/bingqin2/PatchPilot/pull/8',
    latestArchivedAt: '2026-06-29T11:30:00Z',
    downloadActions: [
...
const demoFinalAcceptanceCompletionCloseout = {
  status: 'READY',
  closed: true,
  summary:
    'PatchPilot final acceptance completion is closed with accepted certificates, finalized sharing, and fresh completion delivery proof.',
...
const demoFinalAcceptanceCompletionCloseoutArchive = {
...
const demoFinalExternalReviewReleaseBundleArchive = {
...
const demoHandoffShareDeliveryReceipt = {
...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url === '/api/github/publish-readiness?owner=bingqin2&repository=PatchPilot') {
      return jsonResponse(githubPublishReadiness);
    }
    ...
    if (url === '/api/demo/final-acceptance-completion-closeout') {
      return jsonResponse(demoFinalAcceptanceCompletionCloseout);
    }
    if (url === '/api/demo/final-acceptance-completion-closeout/archives' && init?.method === 'POST') {
    ...
    if (url === '/api/demo/final-acceptance-completion-closeout/archives/final-acceptance-completion-closeout-archive-1/report/download') {
      return Promise.resolve({
        ok: true,
        status: 200,
        blob: async () => new Blob(['# PatchPilot Final Acceptance Completion Closeout Archive'], {
          type: 'text/markdown;charset=UTF-8'
        })
      } as Response);
    }
    if (url === '/api/demo/final-acceptance-completion-closeout/report/download') {
      return Promise.resolve({
        ok: true,
        status: 200,
    ...
    if (url === '/api/demo/final-external-review-release-bundle/archives' && init?.method === 'POST') {
      return jsonResponse(demoFinalExternalReviewReleaseBundleArchive);
    }
    if (url === '/api/demo/final-external-review-release-bundle/archives') {
      return jsonResponse([demoFinalExternalReviewReleaseBundleArchive]);
    }
    if (
      url === '/api/demo/final-external-review-release-bundle/delivery-receipts' &&
      init?.method === 'POST'
    ) {
      return jsonResponse({
        ...demoFinalExternalReviewReleaseBundleDeliveryReceipt,
        id: 'final-external-review-release-bundle-delivery-receipt-2',
        deliveryTarget: 'auditor@example.com',
        notes: 'Sent final release bundle to reviewer.'
      }, true, null, 201);
    }
    if (url === '/api/demo/final-external-review-release-bundle/delivery-receipts') {
      return jsonResponse([demoFinalExternalReviewReleaseBundleDeliveryReceipt]);
    }
    ...
    if (url === '/api/demo/final-external-review-release-bundle/delivery-certificate') {
      return jsonResponse(demoFinalExternalReviewReleaseBundleDeliveryCertificate);
    }
    ...
    if (
      url ===
      '/api/demo/final-external-review-release-bundle/archives/final-external-review-release-bundle-archive-1/report/download'
    ) {
      return Promise.resolve({
        ok: true,
        status: 200,
        blob: async () => new Blob(['# PatchPilot Final External Review Release Bundle Archive'], {
          type: 'text/markdown;charset=UTF-8'
        })
      } as Response);
    }
    if (
      url ===
      '/api/demo/final-external-review-release-bundle/delivery-receipts/final-external-review-release-bundle-delivery-receipt-1/report/download'
    ) {
      return Promise.resolve({
        ok: true,
        status: 200,
        blob: async () => new Blob(['# PatchPilot Final External Review Release Bundle Delivery Receipt'], {
          type: 'text/markdown;charset=UTF-8'
        })
      } as Response);
    }
    if (url === '/api/demo/final-external-review-release-bundle/delivery-finalization/report/download') {
      return Promise.resolve({
        ok: true,
        status: 200,
    ...
    if (url === '/api/demo/final-external-review-release-bundle/delivery-certificate/report/download') {
      return Promise.resolve({
        ok: true,
        status: 200,
        blob: async () => new Blob([
    ...
    if (url === '/api/demo/handoff-share-delivery-receipts' && init?.method === 'POST') {
    ...
    if (url === '/api/demo/handoff-share-delivery-receipts/delivery-receipt-1/report/download') {
      return Promise.resolve({
        ok: true,
        status: 200,
        blob: async () => new Blob(['# PatchPilot Demo Handoff Share Delivery Receipt'], {
          type: 'text/markdown;charset=UTF-8'
        })
      } as Response);
    }
...
test('renders operational task dashboard from backend APIs', async () => {
  ...
  await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/demo/final-acceptance-completion-closeout'));
  ...
  await waitFor(() =>
    expect(fetchMock).toHaveBeenCalledWith('/api/demo/final-external-review-release-bundle/delivery-finalization')
  );
  await waitFor(() =>
    expect(fetchMock).toHaveBeenCalledWith('/api/demo/final-external-review-release-bundle/delivery-certificate')
  );
  await waitFor(() =>
  ...
  expect(within(evidencePanel).getByText('Final external-review release bundle')).toBeInTheDocument();
  expect(within(evidencePanel).getByText('PatchPilot final external-review release bundle is ready.'))
    .toBeInTheDocument();
  expect(within(evidencePanel).getAllByText('final-external-review-delivery-certificate-archive-1').length)
    .toBeGreaterThanOrEqual(1);
  expect(within(evidencePanel).getByText('Final external-review release bundle archive')).toBeInTheDocument();
  expect(within(evidencePanel).getByText('Latest final external-review release bundle archive is frozen and ready.'))
    .toBeInTheDocument();
  expect(within(evidencePanel).getAllByText('final-external-review-release-bundle-archive-1').length)
    .toBeGreaterThanOrEqual(1);
  expect(within(evidencePanel).getByText('Final external-review release bundle delivery')).toBeInTheDocument();
  ...
  expect(within(evidencePanel).getAllByText(
    'final-external-review-release-bundle-delivery-finalization-archive-1'
  ).length).toBeGreaterThanOrEqual(1);
  expect(within(evidencePanel).getByText(
    'Final external-review release bundle delivery certificate archive'
  )).toBeInTheDocument();
  expect(within(evidencePanel).getByText('Certified delivery archive')).toBeInTheDocument();
  expect(within(evidencePanel).getByText(
    'Latest final external-review release bundle delivery certificate archive is certified and ready.'
  )).toBeInTheDocument();
  expect(within(evidencePanel).getAllByText(
  ...
  expect(within(acceptancePanel).getByText('PatchPilot final external-review release bundle archive is frozen.'))
    .toBeInTheDocument();
  expect(within(acceptancePanel).getByRole('heading', {
    name: 'Final external-review release bundle delivery finalization'
  })).toBeInTheDocument();
...
test('summarizes operator setup readiness before a demo run', async () => {
  ...
  expect(within(setupChecklist).getAllByText('GitHub publish readiness')[0]).toBeInTheDocument();
  expect(within(setupChecklist).getByText('Ready - GitHub publish path is ready for PatchPilot push and Pull Request creation.')).toBeInTheDocument();
  expect(within(setupChecklist).getByText('READY - GitHub publish path is ready for PatchPilot push and Pull Request creation.')).toBeInTheDocument();
  expect(within(setupChecklist).getByText('git push origin HEAD:<patchpilot-branch>')).toBeInTheDocument();
  expect(within(setupChecklist).getAllByText('GitHub publish permissions')[0]).toBeInTheDocument();
...
test('records handoff share delivery receipt from the session snapshot panel', async () => {
  const user = userEvent.setup();
  const fetchMock = vi.mocked(fetch);
  ...
  const sessionPanel = await screen.findByRole('region', { name: 'Demo session snapshot' });
  await user.clear(within(sessionPanel).getByLabelText('Delivery target'));
  await user.type(within(sessionPanel).getByLabelText('Delivery target'), 'maintainer@example.com');
  await user.clear(within(sessionPanel).getByLabelText('Operator'));
  await user.type(within(sessionPanel).getByLabelText('Operator'), 'local-operator');
  await user.clear(within(sessionPanel).getByLabelText('Notes'));
  await user.type(within(sessionPanel).getByLabelText('Notes'), 'Sent after the demo review.');
  await user.click(within(sessionPanel).getByRole('button', { name: 'Record handoff share delivery receipt' }));
...
test('downloads handoff share delivery receipt from the session snapshot panel', async () => {
  const user = userEvent.setup();
  const fetchMock = vi.mocked(fetch);
  const click = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(() => {});
  ...
  const sessionPanel = await screen.findByRole('region', { name: 'Demo session snapshot' });
  await user.click(within(sessionPanel).getByRole('button', {
  ...
  expect(within(sessionPanel).getByText('Handoff share delivery receipt downloaded')).toBeInTheDocument();
});
