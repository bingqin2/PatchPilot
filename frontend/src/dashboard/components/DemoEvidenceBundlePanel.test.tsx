...
const bundle: DemoEvidenceBundle = {
  ...
  finalExternalReviewReleaseBundle: {
    ...
    latestDeliveryChannel: 'email',
    latestDeliveredAt: '2026-06-29T05:00:00Z',
    latestCertificateArchivedAt: '2026-06-29T05:30:00Z',
    generatedAt: '2026-06-29T05:35:00Z',
    requiredAttachments: [
    ...
    releaseChecks: [
      {
        name: 'Final external-review delivery certificate archive',
        status: 'READY',
        summary: 'Certified delivery certificate archive final-external-review-delivery-certificate-archive-1 is available.',
        nextAction: 'No action needed.'
      }
    ],
    evidenceNotes: [
  ...
  finalExternalReviewReleaseBundleArchiveEvidence: {
    ...
    latestTaskId: 'task-2',
    latestPullRequestUrl: 'https://github.com/bingqin2/PatchPilot/pull/42',
    latestArchivedAt: '2026-06-29T05:45:00Z',
    downloadActions: [
  ...
  finalExternalReviewReleaseBundleDeliveryFinalization: {
    ...
    nextAction: 'Use this finalization report as proof that the frozen release bundle was delivered.',
    latestArchiveId: 'final-external-review-release-bundle-archive-1',
    ...
    latestDeliveryTarget: 'reviewer@example.com',
    latestDeliveryChannel: 'email',
    latestDeliveredAt: '2026-06-29T05:50:00Z',
    ...
    checks: [
      {
        ...
        status: 'READY',
        summary: 'Frozen final external-review release bundle archive is ready.',
        nextAction: 'No action needed.'
      }
    ],
    evidenceNotes: [
...
test('summarizes demo evidence bundle for operators', async () => {
  ...
  expect(within(panel).getByText('Final external-review release bundle')).toBeInTheDocument();
  expect(within(panel).getByText('Release ready')).toBeInTheDocument();
  expect(within(panel).getByText('PatchPilot final external-review release bundle is ready.')).toBeInTheDocument();
  expect(
    within(panel).getByText('Share the release bundle report and listed attachments with external reviewers.')
  ).toBeInTheDocument();
  expect(
    within(panel).getAllByText('final-external-review-delivery-certificate-archive-1').length
  ).toBeGreaterThanOrEqual(1);
  expect(
    within(panel).getAllByText('final-external-review-package-delivery-finalization-archive-1').length
  ).toBeGreaterThanOrEqual(2);
  expect(
    within(panel).getByRole('link', { name: 'Open final external-review release Pull Request' })
  ).toHaveAttribute('href', 'https://github.com/bingqin2/PatchPilot/pull/42');
  expect(
    within(panel).getByText('Final external-review delivery certificate archive final-external-review-delivery-certificate-archive-1.')
  ).toBeInTheDocument();
  expect(within(panel).getByText('Download final external-review release bundle report.')).toBeInTheDocument();
  expect(
    within(panel).getAllByText(
      'Download final external-review delivery certificate archive final-external-review-delivery-certificate-archive-1.'
    ).length
  ).toBeGreaterThanOrEqual(1);
  expect(within(panel).getByText('Final external-review release bundle archive')).toBeInTheDocument();
  expect(within(panel).getByText('Release bundle archive ready')).toBeInTheDocument();
  expect(
    within(panel).getByText('Latest final external-review release bundle archive is frozen and ready.')
  ).toBeInTheDocument();
  expect(
    within(panel).getByText('Use the archived final external-review release bundle as the reviewer-facing source.')
  ).toBeInTheDocument();
  expect(
    within(panel).getAllByText('final-external-review-release-bundle-archive-1').length
  ).toBeGreaterThanOrEqual(1);
  expect(
    within(panel).getAllByText(
      'Download final external-review release bundle archive final-external-review-release-bundle-archive-1.'
  ...
  expect(within(panel).getByText('Final external-review release bundle delivery')).toBeInTheDocument();
  expect(
...
test('renders missing certificate evidence for legacy bundle responses', () => {
  ...
  expect(screen.getAllByText('No archived final external-review Pull Request').length)
    .toBeGreaterThanOrEqual(1);
  ...
  expect(screen.getByText('Final external-review release bundle')).toBeInTheDocument();
  expect(screen.getByText('No final external-review release bundle is available.')).toBeInTheDocument();
  expect(
    screen.getByText('Archive the certified final external-review delivery certificate, then download the release bundle.')
  ).toBeInTheDocument();
  expect(screen.getByText('No final external-review release Pull Request')).toBeInTheDocument();
  expect(screen.getByText('Final external-review release bundle archive')).toBeInTheDocument();
  expect(screen.getByText('No final external-review release bundle archive is available.')).toBeInTheDocument();
  expect(
    screen.getAllByText('Archive the READY final external-review release bundle.').length
  ).toBeGreaterThanOrEqual(1);
