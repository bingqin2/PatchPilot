...
- Accepted tasks should expose a pre-execution safety snapshot in task detail, copied reports, and the dashboard with source, final allow decision, safety-gate result, quarantine state, rate-limit state, issue-context state, model trigger-classification result, and evidence timestamp.
