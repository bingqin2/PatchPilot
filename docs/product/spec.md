...
- Operators should be able to create and download one final live trigger launch package after a ready live launch gate and a frozen external exposure operator handoff archive exist. `POST /api/demo/live-trigger-launch-package` must accept the exact repository, issue, trigger user, and `/agent fix` comment, combine the live launch gate with the latest operator handoff archive, and block posting when that archive is missing or not ready. `POST /api/demo/live-trigger-launch-package/report/download` must return the same package as Markdown. Both endpoints are read-only and must not create tasks, call the model directly, enqueue work, mutate Git, create branches, open Pull Requests, write GitHub comments, archive records, or expose secrets.
- Operators should be able to inspect one read-only end-to-end acceptance matrix that summarizes whether PatchPilot is ready for the final self-hosted issue-to-PR demo. The matrix must aggregate live launch gate status, supported language coverage, safety rejection coverage, evaluation baseline and run evidence, recent successful Pull Request evidence, recent failure handling evidence, pending-review safety evidence, final product gap analysis, counts, next actions, side-effect contract, and copyable Markdown through `GET /api/demo/end-to-end-acceptance-matrix` without creating tasks, calling the model, running tests, mutating Git, creating branches, opening Pull Requests, posting comments, archiving records, or writing to GitHub.
- Operators should be able to inspect one read-only external exposure readiness gate before exposing a local backend through Cloudflare Tunnel or another temporary public URL. The gate must aggregate admin-token protection, disabled dashboard token bootstrap, webhook secret presence, public webhook URL configuration, trigger user and repository allowlists, trigger rate limiting, rejected-trigger quarantine, review approval allowlist, generated-diff risk gating, counts, next actions, side-effect contract, and copyable Markdown through `GET /api/security/external-exposure-readiness` without creating tasks, calling the model, probing the network, running tests, mutating Git, opening Pull Requests, writing GitHub comments, archiving records, or exposing secrets.
- Operators should be able to explicitly archive the current external exposure readiness result as PatchPilot-local evidence before sharing a temporary public URL. The archive flow must capture status, safe-to-expose flag, counts, timestamp, summary, and Markdown report through `POST /api/security/external-exposure-readiness/archives`, list recent archives through `GET /api/security/external-exposure-readiness/archives`, and download exact archived Markdown through `GET /api/security/external-exposure-readiness/archives/{archiveId}/report/download` without creating tasks, calling the model, probing the network, running tests, mutating Git, opening Pull Requests, writing GitHub comments, or exposing secrets.
- Operators should be able to inspect and download a read-only external exposure handoff package before sharing a temporary public URL. The package must combine the current exposure readiness gate with the latest archived readiness evidence, report whether the archive is missing, stale, or current, expose a `READY`, `NEEDS_ATTENTION`, or `BLOCKED` handoff status, and return Markdown through `GET /api/security/external-exposure-handoff-package` and `GET /api/security/external-exposure-handoff-package/report/download` without creating tasks, calling the model, probing the network, running tests, mutating Git, opening Pull Requests, writing GitHub comments, archiving records, or exposing secrets.
- Operators should be able to record the active lifetime of a temporary public URL after the external exposure handoff package is `READY`. The session flow must create local evidence through `POST /api/security/external-exposure-sessions`, list recent sessions through `GET /api/security/external-exposure-sessions`, close a session through `POST /api/security/external-exposure-sessions/{sessionId}/close`, and download Markdown evidence through `GET /api/security/external-exposure-sessions/{sessionId}/report/download`. Session creation must capture public URL, webhook URL, purpose, operator, optional expected shutdown time, notes, linked handoff status, and linked readiness archive id. Session records must not probe the URL, edit GitHub webhook settings, create tasks, call the model, run tests, mutate Git, open Pull Requests, write GitHub comments, archive readiness records, or expose secrets.
- Operators should be able to inspect and download one read-only external exposure closeout gate after a temporary public URL demo or smoke test. The gate must return `READY` only when the latest session is `CLOSED`, has closer, close time, close notes, a linked readiness archive id, and the current handoff package is `READY`; it must return `BLOCKED` while the latest session is still `ACTIVE`, and `NEEDS_ATTENTION` when close evidence or handoff readiness is incomplete. It must expose JSON and Markdown through `GET /api/security/external-exposure-closeout` and `GET /api/security/external-exposure-closeout/report/download` without probing public URLs, editing GitHub webhook settings, creating tasks, calling the model, running tests, mutating Git, opening Pull Requests, writing GitHub comments, archiving records, or exposing secrets.
- Operators should be able to archive, list, and download exact external exposure closeout snapshots as PatchPilot-local evidence after the temporary public URL is shut down. Archive creation must freeze the current closeout status, closeout-ready flag, latest session evidence, linked readiness archive, handoff status, archive freshness, counts, generated time, archived time, evidence notes, next actions, download actions, side-effect contract, and Markdown report through `POST /api/security/external-exposure-closeout/archives`, list recent archives through `GET /api/security/external-exposure-closeout/archives`, and download frozen Markdown through `GET /api/security/external-exposure-closeout/archives/{archiveId}/report/download` without probing public URLs, editing GitHub webhook settings, creating tasks, calling the model, running tests, mutating Git, opening Pull Requests, writing GitHub comments, sending messages, or exposing secrets.
- Operators should be able to verify the latest external exposure closeout archive directly from the top-level demo evidence bundle, copied runbook, and dashboard evidence bundle panel. That readout must include archive status, closeout-ready flag, archive id, session id/status, public URL, webhook URL, linked readiness archive id, archive freshness, next action, and download actions without creating archives, probing public URLs, editing GitHub webhook settings, creating tasks, calling the model, running tests, mutating Git, opening Pull Requests, writing GitHub comments, sending messages, or exposing secrets.
- Operators should be able to archive, list, and download the current external exposure operator handoff checklist as PatchPilot-local evidence before posting the next live `/agent fix` trigger. Archive creation must freeze the checklist status, go/no-go flag, latest closeout archive, session state, public URL, webhook URL, handoff status, live publish status, check counts, generated time, archived time, evidence notes, next actions, download actions, side-effect contract, and Markdown report through `POST /api/security/external-exposure-operator-handoff-checklist/archives`, list recent archives through `GET /api/security/external-exposure-operator-handoff-checklist/archives`, and download frozen Markdown through `GET /api/security/external-exposure-operator-handoff-checklist/archives/{archiveId}/report/download` without creating tasks, calling the model, probing public URLs, mutating Git, opening Pull Requests, writing GitHub comments, sending messages, or exposing secrets.
