...
- Expose a read-only external exposure operator handoff checklist through `/api/security/external-exposure-operator-handoff-checklist` and `/api/security/external-exposure-operator-handoff-checklist/report/download`. This read model composes the latest closeout archive, current exposure handoff package, recent exposure sessions, and live GitHub publish preflight for the configured demo repository into one go/no-go result before the next live `/agent fix` trigger. A capped process-local archive store freezes recent checklist snapshots through `POST /api/security/external-exposure-operator-handoff-checklist/archives`, `GET /api/security/external-exposure-operator-handoff-checklist/archives`, and archived Markdown downloads so operators can reopen the exact go/no-go evidence after runtime state changes.
- Expose a read-only live trigger launch package through `POST /api/demo/live-trigger-launch-package` and `POST /api/demo/live-trigger-launch-package/report/download`. This package reuses the live launch gate command, combines the current live launch gate with the latest external exposure operator handoff checklist archive, and returns `READY`, `NEEDS_ATTENTION`, or `BLOCKED` before the operator posts the exact GitHub issue comment. It blocks when the operator handoff archive is missing or not ready, includes the exact issue URL/comment, archive id, live-gate status, evidence notes, next actions, side-effect contract, and Markdown report, and does not create tasks or write new archive state.
