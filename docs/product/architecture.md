...
`POST /api/repository-preflight` accepts a local repository path, resolves it from the backend working directory when relative, and runs only adapter detection. It returns supported status, language, build system, verification command, detection reason, operator action, and the supported adapter catalog for unsupported results. This endpoint is an operator diagnostic for local fixtures and workspaces; it does not create a task, run verification commands, mutate Git, call the model, or write to GitHub.
