...
- Store a separate capped archive of recent demo handoff packages so the final post-demo package is preserved independently from session reports. Each package archive also stores the computed handoff readiness status, summary, next action, and ready/warning/blocked check counts from archive time. The same profile split applies: the default profile keeps package archives in memory, and `local`, `docker`, and `idea` profiles persist them through MySQL.
