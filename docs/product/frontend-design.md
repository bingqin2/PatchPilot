...
- `TriggerDecisionPanel`: selected accepted-trigger evidence beside recent rejected trigger decisions and category summary for quick operator comparison.
...
- Trigger decision visibility shows why the selected `/agent` comment was accepted and what recent comments were refused instead, using the selected task's `TRIGGER_ACCEPTED` timeline event plus recent rejected-trigger rows.
