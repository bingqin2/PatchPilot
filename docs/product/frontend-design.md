...
- `RejectedTriggerPanel`: read-only recent `/agent fix` attempts that were refused before task creation, including command text, repository, trigger user, delivery id, stable rejection category, rejection reason, a category filter, and a compact recent-window summary by category, source, top trigger users, and top repositories. It also surfaces `ABUSE_QUARANTINED` rows created by repeated rejected-trigger patterns.
