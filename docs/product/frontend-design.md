...
It also loads `GET /api/demo/evidence-bundle`, `GET /api/demo/session-snapshot`, `GET /api/demo/session-archives`, `GET /api/demo/script`, `GET /api/demo/readiness`, `GET /api/demo/smoke-checklist`, `GET /api/language-adapters`, `GET /api/language-adapters/fixtures`, `GET /api/rejected-triggers`, `GET /api/rejected-triggers/summary`, `GET /api/trigger-quarantines`, `GET /api/operator-safety-audits`, and `GET /api/github/webhook-deliveries` so demo evidence, a full demo session snapshot, recent archived session reports, the read-only demo script, readiness, final smoke-check steps, supported repository shapes, fixed verification commands, detection signals, demo fixture paths, current fixture pass/fail status, selected accepted-trigger evidence, recent rejected task triggers, rejected-trigger abuse patterns, active trigger quarantines, recent manual safety mutations, recent GitHub delivery outcomes, and redelivery guidance are visible in the dashboard instead of only in source code, terminal smoke output, curl output, or GitHub's delivery page. It calls `GET /api/demo/runbook` only when an operator clicks `Copy runbook`, `GET /api/demo/session-report` only when an operator clicks `Copy session report`, `GET /api/demo/session-report/download` only when an operator clicks `Download session report`, `GET /api/demo/session-archives/{archiveId}/report/download` only when an operator downloads one archived report, and `POST /api/demo/session-archives` only when an operator clicks `Archive session`, so Markdown reports and archive records are created or retrieved from current backend evidence without adding another task-triggering path. The trigger-decision panel reuses the selected task timeline and rejected-trigger data to compare accepted and rejected decisions in one place. The rejected-trigger panel also posts manual trigger quarantine create and release actions to the same protected quarantine API so operators can block abusive trigger users or repositories and recover false positives without leaving the dashboard.
