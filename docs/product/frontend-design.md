...
It also loads `GET /api/demo/evidence-bundle`, `GET /api/demo/readiness`, `GET /api/demo/smoke-checklist`, `GET /api/language-adapters`, `GET /api/language-adapters/fixtures`, `GET /api/rejected-triggers`, `GET /api/rejected-triggers/summary`, `GET /api/trigger-quarantines`, `GET /api/operator-safety-audits`, and `GET /api/github/webhook-deliveries` so demo evidence, readiness, final smoke-check steps, supported repository shapes, fixed verification commands, detection signals, demo fixture paths, current fixture pass/fail status, recent rejected task triggers, rejected-trigger abuse patterns, active trigger quarantines, recent manual safety mutations, recent GitHub delivery outcomes, and redelivery guidance are visible in the dashboard instead of only in source code, terminal smoke output, curl output, or GitHub's delivery page. The rejected-trigger panel also posts manual trigger quarantine create and release actions to the same protected quarantine API so operators can block abusive trigger users or repositories and recover false positives without leaving the dashboard.
...
- `DemoEvidenceBundlePanel`: top-level demo evidence summary that combines readiness, smoke checklist, adapter fixture counts, queue state, recent task/PR evidence, latest webhook delivery, rejected-trigger counts, and active quarantine count.
