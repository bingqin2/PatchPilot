...
- `TaskDetailPanel`: selected task summary, copyable task deep link, copyable task report, queue state and queue history, execution evidence strip, generated-diff preview, approval audit metadata/form, timeline, test runs, tool calls, model calls, cancel/retry/approve-review actions.
