...
- `OperatorSetupChecklistPanel`: read-only demo setup checklist that turns connectivity, required credentials, GitHub credential readiness, safety policy, adapter fixtures, queue health, worker heartbeat readiness, and recent Pull Request evidence into ready/attention checks with next actions.
