...
- `WebhookDeliveryPanel`: read-only recent delivery diagnostics for ignored comments, rejected triggers, duplicate deliveries, invalid signatures, bad payloads, active-task collisions, and task-created outcomes, including whether a delivery should be fixed and redelivered from GitHub. It also provides a pasted-payload diagnostic form backed by `POST /api/github/webhook-diagnostics/evaluate-payload` so operators can check payload JSON, signature status, event/action support, `/agent fix` recognition, parsed target fields, and the next action before redelivery.
