...
- `TaskDetailPanel`: selected task summary, copyable task deep link, copyable task report, queue state and queue history, execution evidence strip, generated-diff preview, timeline, test runs, tool calls, model calls, cancel/retry/approve-review actions.
...
- Pending-review tasks expose an explicit approve-review action that resumes the already-generated workspace after operator review.
