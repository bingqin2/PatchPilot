...
It also loads `GET /api/demo/readiness`, `GET /api/demo/smoke-checklist`, `GET /api/language-adapters`, `GET /api/language-adapters/fixtures`, `GET /api/rejected-triggers`, `GET /api/rejected-triggers/summary`, and `GET /api/github/webhook-deliveries` so demo readiness, final smoke-check steps, supported repository shapes, fixed verification commands, detection signals, demo fixture paths, current fixture pass/fail status, recent rejected task triggers, rejected-trigger abuse patterns, recent GitHub delivery outcomes, and redelivery guidance are visible in the dashboard instead of only in source code, terminal smoke output, curl output, or GitHub's delivery page.
...
- `RejectedTriggerPanel`: read-only recent `/agent fix` attempts that were refused before task creation, including command text, repository, trigger user, delivery id, stable rejection category, rejection reason, a category filter, and a compact recent-window summary by category, source, top trigger users, and top repositories.
...
- Rejected-trigger visibility shows why a `/agent fix` comment did not create a task when the safety gate, allowlists, rate limiter, or model classifier intentionally refused it. Each row shows a category badge, and the panel can filter by category through the URL-backed `rejectedCategory` parameter so operators can isolate vague requests, dangerous instructions, unauthorized users, blocked repositories, rate limits, and model-classifier refusals without parsing reason text. The panel also summarizes the recent rejection window by category, source, top trigger users, and top repositories; category summary rows are clickable and reuse the same filter.
...
- Make safety decisions observable by showing rejected trigger audit records and recent rejection-pattern summaries in the same control plane as accepted tasks.
