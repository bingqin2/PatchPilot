...
- `ManualTaskForm`: local demo/debug trigger evaluation through `POST /api/tasks/evaluate-trigger` plus task creation through `POST /api/tasks`, with explicit repository, issue, trigger user, `/agent fix` command fields, and a preview-source selector for manual API versus GitHub issue-comment gate behavior.
...
- A manual task form can evaluate a local demo/debug `/agent fix` trigger before creation, switch the dry-run source between manual API and GitHub issue-comment gate behavior, then enqueue the task through the same backend task and queue path used by webhook-created tasks.
...
- Manual trigger evaluation visibility shows whether a proposed local `/agent fix` would create a task or be blocked, which source was evaluated, which gate made that decision, whether issue context was loaded for model classification, and the next operator action. It is read-only and does not create tasks, queue items, rejected-trigger audit rows, GitHub comments, webhook delivery diagnostics, or rate-limit records.
...
- Let operators dry-run a manual `/agent fix` trigger before task creation, using the same backend gates without mutating task, queue, GitHub, rejected-trigger, or rate-limit state.
