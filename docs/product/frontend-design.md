...
It calls `POST /api/demo/launch-preflight` only when an operator submits the launch preflight form, combining current demo readiness with a read-only GitHub issue-comment trigger dry run for the exact `/agent fix` comment that will be posted.
