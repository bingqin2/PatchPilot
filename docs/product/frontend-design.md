...
- Cancel and retry are available only for task states where those actions make sense; failed and cancelled tasks show retry preflight before queuing another attempt, and retried tasks keep source failure context in the detail panel.
...
- Prevent blind task retries by showing task retry preflight and disabling retry when setup or repository support work is required first.
