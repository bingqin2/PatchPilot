...
- `ManualTaskForm`: local demo/debug trigger evaluation through `POST /api/tasks/evaluate-trigger` plus task creation through `POST /api/tasks`, with explicit repository, issue, trigger user, and `/agent fix` command fields.
...
- A manual task form can evaluate a local demo/debug `/agent fix` trigger before creation, then enqueue the task through the same backend task and queue path used by webhook-created tasks.
...
- Manual trigger evaluation visibility shows whether a proposed local `/agent fix` would create a task or be blocked, which gate made that decision, whether issue context was loaded for model classification, and the next operator action. It is read-only and does not create tasks, queue items, rejected-trigger audit rows, GitHub comments, or rate-limit records.
