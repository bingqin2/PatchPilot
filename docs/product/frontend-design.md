...
- `RejectedTriggerPanel`: read-only recent `/agent fix` attempts that were refused before task creation, including command text, repository, trigger user, delivery id, stable rejection category, and rejection reason.
...
- Rejected-trigger visibility shows why a `/agent fix` comment did not create a task when the safety gate, allowlists, rate limiter, or model classifier intentionally refused it. Each row shows a category badge so operators can distinguish vague requests, dangerous instructions, unauthorized users, blocked repositories, rate limits, and model-classifier refusals without parsing reason text.
