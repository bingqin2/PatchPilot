...
- The first implementation surface is an evaluation case catalog, readiness summary, fixture-readiness report, on-demand fixture execution baseline, fixture baseline run archive, and run preview API/dashboard panel, so operators can inspect supported language and safety-rejection scenarios, verify checked-in fixture coverage, run adapter-selected local fixture commands, preserve baseline execution reports, and copy benchmark-shaped evidence before persisted automated benchmark runs exist.
