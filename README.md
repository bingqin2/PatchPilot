...
- Read-only evaluation case catalog and readiness summary for Java/Maven, Node/npm, Python/pytest, Go, unsafe-trigger rejection, and vague-trigger rejection scenarios, with expected commands, expected files, success criteria, safety expectations, next action, and copyable Markdown evidence.
...
These endpoints return benchmark-ready cases for supported issue-to-PR fixes and safety rejections plus a readiness summary with total case count, supported fix count, safety rejection count, covered languages, covered build systems, rejection categories, and next action. They do not create tasks, call the model, run tests, mutate Git, or write to GitHub. The dashboard shows the same catalog and readiness summary in `Evaluation case catalog` and can copy a Markdown report for demo handoff or review notes.
