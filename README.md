...
- Copyable and downloadable demo session report Markdown that exports the full session snapshot, prepared demo launch commands from the current browser, script, checklist, health contract, next actions, and embedded runbook.
...
The demo session report endpoint formats that session snapshot as Markdown for copy/paste handoff. It includes session status, share summary, recent task and Pull Request evidence, prepared launch commands when supplied by the dashboard, operator checklist, script steps, health contract, next actions, and the embedded runbook. `GET /api/demo/session-report` and `GET /api/demo/session-report/download` keep read-only compatibility without browser-local context; the dashboard uses `POST /api/demo/session-report` and `POST /api/demo/session-report/download` to include current prepared command history. These endpoints do not create tasks, call the model, run tests, mutate Git, or write to GitHub.
The demo session archive endpoints store and list the most recent session reports for local demos. `POST /api/demo/session-archives` captures the current snapshot and Markdown report, including dashboard-supplied prepared launch commands when present, while `GET /api/demo/session-archives` lists the latest 20 archives. The default database-free profile keeps archives in memory; `local`, `docker`, and `idea` profiles persist them in MySQL through Flyway/MyBatis. Archive creation is PatchPilot-local state only: it does not create tasks, call the model, run tests, mutate Git, or write to GitHub.
...
The demo session snapshot combines the current evidence bundle, browser-local prepared launch commands, script, runbook, operator checklist, health contract, share summary, and next actions in one panel so an operator can confirm the demo state before or after the live `/agent fix` trigger. Its `Copy session report` and `Download session report` actions fetch Markdown reports on demand with the current prepared command context for review notes, issue comments, or handoff messages. Its `Archive session` action captures the current report into the archive store and lists recent archived reports with their own copy/download actions.
