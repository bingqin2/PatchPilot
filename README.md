...
The demo handoff package endpoints format a higher-level Markdown artifact for post-demo sharing. `POST /api/demo/handoff-package` returns a copyable package, and `POST /api/demo/handoff-package/download` returns the same package as a Markdown attachment. The package includes a concise handoff summary, a handoff readiness check, readiness trend evidence, recent webhook delivery evidence, recent task and Pull Request evidence, prepared-command and archived-outcome counts, next actions, prepared launch commands, archived launch outcomes, and the embedded session report. The readiness check reports `READY`, `NEEDS_ATTENTION`, or `BLOCKED` based on demo status, recent task-created webhook delivery evidence, recent completed task evidence, Pull Request evidence, prepared command context, archived outcome evidence, and readiness trend baseline. A recent delivery that requires redelivery, such as an invalid signature or backend processing failure, blocks the handoff until the operator fixes the setup and redelivers from GitHub. These endpoints are read-only and do not create tasks, call the model, run tests, mutate Git, archive records, or write to GitHub.
