...
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, latest handoff share checklist status, final handoff share center evidence, latest handoff share delivery receipt evidence, and handoff finalization status into one status, count set, and next-action list. Its evidence cards show whether the GitHub payload URL, secret-backed setup, and post-demo share evidence are ready, including which final handoff artifacts should be downloaded, whether a delivery receipt is missing, fresh for the current archive/session, or stale from an older package, and whether the handoff is finalized before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff.
