...
The final demo acceptance panel is backed by `GET /api/demo/acceptance-summary`, `GET /api/demo/acceptance-summary/report/download`, `GET /api/demo/final-acceptance-share-package`, `GET /api/demo/final-acceptance-share-package/report/download`, and the final acceptance archive, receipt, finalization, completion archive, and completion evidence bundle endpoints. It gives the operator one final accepted/not-accepted answer across launch and task certificates, shows the linked archive ids and checks, downloads a portable Markdown summary, presents a reviewer-facing share package with recipients, attachments, pre-send checks, prepared subject/body text, copy action, Markdown download, local delivery receipt recording, finalization proof, final completion archive history, and one downloadable final completion evidence bundle. The share package and completion archive actions are operator aids; PatchPilot does not send external messages from this panel.
