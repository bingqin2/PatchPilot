...
- Read-only evaluation case catalog, readiness summary, and run preview report for Java/Maven, Node/npm, Python/pytest, Go, unsafe-trigger rejection, and vague-trigger rejection scenarios, with expected commands, expected files, success criteria, safety expectations, known gaps, next action, and copyable Markdown evidence.
...
These endpoints return benchmark-ready cases for supported issue-to-PR fixes and safety rejections, a readiness summary, and a benchmark-shaped preview report with expected verification commands, known gaps, side-effect contract, and next action. They do not create tasks, call the model, clone repositories, run tests, mutate Git, or write to GitHub. The preview report is derived from checked-in metadata only; it is not a real benchmark run. The dashboard shows the same catalog, readiness summary, and run preview in `Evaluation case catalog` and can copy Markdown reports for demo handoff or review notes.
