...
Use `/detail` for dashboard-style task inspection. It returns the task audit summary, latest queue item, queue history, latest generated diff, failed-task diagnosis, timeline events, test runs, tool calls, and model calls in one response. Use `/report` to copy a Markdown diagnostic summary for a task, including the failure diagnosis and generated diff when captured. The narrower endpoints remain available for focused debugging.
