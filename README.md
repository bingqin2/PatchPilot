...
The evidence bundle endpoint is the one-call demo summary. It includes readiness, smoke checklist, non-sensitive configuration, adapter fixture counts, full evaluation run readiness, queue summary, recent task and Pull Request evidence, combined webhook setup readiness, latest webhook delivery, a capped recent webhook delivery trail with task/rejected-trigger/error outcomes, rejected-trigger counts, active quarantine count, latest handoff share checklist status, final handoff share center status, share-center download actions, latest handoff share delivery receipt summary and freshness state, final handoff finalization status, final launch evidence share-center status, final launch evidence finalization status, generated time, and next actions. The overall evidence-bundle status is not `READY` until the full evaluation archive evidence, handoff finalization gate, and launch evidence finalization gate are `READY`.
...
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, full evaluation run readiness, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, latest handoff share checklist status, final handoff share center evidence, latest handoff share delivery receipt evidence, handoff finalization status, final launch evidence share-center status, and final launch evidence finalization status into one status, count set, and next-action list. Its evidence cards show whether the latest full evaluation archive is ready, which languages/build systems and safety rejection categories are covered, whether the GitHub payload URL, secret-backed setup, post-demo share evidence, archived launch evidence, and accepted launch finalization evidence are ready, which final handoff and launch evidence artifacts should be downloaded, whether a delivery receipt is missing, fresh for the current archive/session, or stale from an older package, and whether the handoff and launch evidence are finalized before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff, including launch evidence share-center and acceptance status.
