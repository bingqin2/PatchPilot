...
The final demo acceptance panel is backed by `GET /api/demo/acceptance-summary`, `GET /api/demo/acceptance-summary/report/download`, `GET /api/demo/final-acceptance-share-package`, `GET /api/demo/final-acceptance-share-package/report/download`, and the final acceptance archive, receipt, finalization, completion archive, completion evidence bundle, completion evidence delivery receipt, and completion delivery finalization endpoints. It gives the operator one final accepted/not-accepted answer across launch and task certificates, shows the linked archive ids and checks, downloads a portable Markdown summary, presents a reviewer-facing share package with recipients, attachments, pre-send checks, prepared subject/body text, copy action, Markdown download, local delivery receipt recording, finalization proof, final completion archive history, one downloadable final completion evidence bundle, a local form/history for recording where that final completion evidence bundle was delivered, and a read-only finalization gate proving whether the latest receipt closes the current completion bundle. Completion evidence delivery receipts are created through `POST /api/demo/final-acceptance-completion-evidence-delivery-receipts`, listed through `GET /api/demo/final-acceptance-completion-evidence-delivery-receipts`, and downloaded through `GET /api/demo/final-acceptance-completion-evidence-delivery-receipts/{receiptId}/report/download`; completion delivery finalization is loaded through `GET /api/demo/final-acceptance-completion-evidence-delivery-finalization` and downloaded through `GET /api/demo/final-acceptance-completion-evidence-delivery-finalization/report/download`. The share package, completion archive, and completion evidence receipt actions are operator aids; PatchPilot does not send external messages from this panel.
