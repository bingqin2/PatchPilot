...
Use `/detail` for dashboard-style task inspection. It returns the task audit summary, latest queue item, queue history, accepted-trigger intent audit, persisted pre-execution safety snapshot, latest generated diff, failed-task diagnosis, timeline events, test runs, tool calls, and model calls in one response. Use `/report` to copy a Markdown diagnostic summary for a task, including accepted-trigger intent, pre-execution safety evidence, failure diagnosis, and generated diff when captured. Use `/api/tasks/evidence-packages`, `/api/tasks/evidence-packages/summary`, and `/api/tasks/evidence-packages/share-center` to review recent archived task evidence across tasks, aggregate counts, and determine whether a completed Pull Request-backed archive is ready to share. `GET /api/tasks/evidence-packages/share-center/report/download` downloads the same share-center decision as Markdown. These archive review and share endpoints are read-only; they do not create tasks, call the model, run verification commands, mutate Git, push, open Pull Requests, or write GitHub comments. The narrower endpoints remain available for focused debugging.
