...
Failure-cause metrics use the same stable categories and next-action guidance as failed-task issue feedback, so repeated failures are grouped as `VERIFICATION_FAILED`, `GITHUB_OPERATION_FAILED`, `MODEL_FAILED`, `WORKSPACE_FAILED`, `UNSUPPORTED_REPOSITORY`, `PATCH_REVIEW_REJECTED`, or `TASK_FAILED` instead of ad hoc exception text.
