...
The rejected-trigger panel shows recent `/agent fix` comments that were intentionally refused before task creation, including the command text, rejection reason, recent category/source counts, and top trigger users/repositories. Use it when GitHub shows a successful delivery but no task appears or when repeated unsafe traffic needs investigation.
