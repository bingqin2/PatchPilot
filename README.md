...
The final demo acceptance panel is backed by `GET /api/demo/acceptance-summary`, `GET /api/demo/acceptance-summary/report/download`, `GET /api/demo/final-acceptance-share-package`, and `GET /api/demo/final-acceptance-share-package/report/download`. It gives the operator one final accepted/not-accepted answer across launch and task certificates, shows the linked archive ids and checks, downloads a portable Markdown summary, and presents a reviewer-facing share package with recipients, attachments, pre-send checks, prepared subject/body text, copy action, and Markdown download. The share package is still a read-only operator aid; PatchPilot does not send external messages from this panel.
