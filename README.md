...
The fixture baseline run archive endpoint executes the same checked-in fixture baseline and stores the resulting Markdown report as PatchPilot-local evidence. Recent baseline runs can be listed or downloaded through the API and dashboard; archived baseline runs still do not create tasks, call the model, mutate Git, push branches, open Pull Requests, or write to GitHub.
