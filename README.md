...
- Copyable and downloadable demo handoff package Markdown that wraps the session report with a concise summary, readiness trend evidence, prepared command counts, archived outcome counts, task evidence, Pull Request evidence, and next actions.
...
The demo handoff package endpoints format a higher-level Markdown artifact for post-demo sharing. `POST /api/demo/handoff-package` returns a copyable package, and `POST /api/demo/handoff-package/download` returns the same package as a Markdown attachment. The package includes a concise handoff summary, readiness trend evidence, recent task and Pull Request evidence, prepared-command and archived-outcome counts, next actions, prepared launch commands, archived launch outcomes, and the embedded session report. These endpoints are read-only and do not create tasks, call the model, run tests, mutate Git, archive records, or write to GitHub.
