...
The dashboard also stores `status` and `query` in the browser URL, so links like `/tasks/{taskId}?status=FAILED&query=maven` reopen the same filtered investigation view. The `Clear filters` action removes only `status` and `query`, keeping the selected task route, unrelated query parameters, and hash fragments intact.
