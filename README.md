...
The dashboard task list requests `query`, `status`, `repositoryOwner`, `repositoryName`, `sort`, `limit`, and `offset` from the backend and consumes the task page response with `items`, `limit`, `offset`, `hasMore`, and `total`.
The dashboard also stores `status`, `query`, repository filters, and non-default `sort` in the browser URL, so links like `/tasks/{taskId}?status=FAILED&query=maven&repositoryOwner=bingqin2&repositoryName=PatchPilot&sort=createdAtAsc` reopen the same filtered investigation view. The `Clear filters` action removes `status`, `query`, `repositoryOwner`, and `repositoryName`, keeping sort, the selected task route, unrelated query parameters, and hash fragments intact.
