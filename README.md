...
- Read-only evaluation case catalog for Java/Maven, Node/npm, Python/pytest, Go, unsafe-trigger rejection, and vague-trigger rejection scenarios, with expected commands, expected files, success criteria, safety expectations, and copyable Markdown evidence.
...
This endpoint returns benchmark-ready cases for supported issue-to-PR fixes and safety rejections, including repository fixture path, issue text, expected adapter metadata, allowlisted verification command, expected changed files, success criteria, expected decision, and safety expectation. It does not create tasks, call the model, run tests, mutate Git, or write to GitHub. The dashboard shows the same catalog in `Evaluation case catalog` and can copy a Markdown report for demo handoff or review notes.
