...
The dashboard also stores `status` and `query` in the browser URL, so links like `/tasks/{taskId}?status=FAILED&query=maven` reopen the same filtered investigation view.
