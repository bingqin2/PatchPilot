...
The demo handoff package archive endpoints store the final handoff package separately from session-report archives. `POST /api/demo/handoff-package-archives` captures the current snapshot, generated handoff package, handoff readiness status, readiness summary, next action, and ready/warning/blocked check counts; `GET /api/demo/handoff-package-archives` lists the latest 20 package archives; `GET /api/demo/handoff-package-archives/summary` returns the latest archive's share-ready status, archive count, latest readiness status, next action, and Markdown evidence report; `GET /api/demo/handoff-package-archives/summary-report/download` downloads that summary report as a Markdown attachment; and `GET /api/demo/handoff-package-archives/{archiveId}/report/download` downloads one archived package. The default profile keeps package archives in memory; `local`, `docker`, and `idea` profiles persist them in MySQL. Archive creation records a protected admin audit row and remains PatchPilot-local state only. The summary endpoints are read-only and do not create tasks, call the model, run tests, mutate Git, archive records, or write to GitHub.
...
The demo session snapshot combines the current evidence bundle, readiness snapshot trend, browser-local prepared launch commands, browser-local archived launch outcomes, script, runbook, operator checklist, health contract, share summary, and next actions in one panel so an operator can confirm both the current demo state and readiness movement before or after the live `/agent fix` trigger. Its `Copy session report` and `Download session report` actions fetch Markdown reports on demand with the current readiness trend, prepared command, and outcome archive context for review notes, issue comments, or handoff messages. Its `Copy handoff package` and `Download handoff package` actions fetch a higher-level Markdown package with summary counts, webhook delivery readiness, readiness trend evidence, task/PR evidence, next actions, prepared commands, archived outcomes, and the embedded session report. Its archive actions capture the current session report or final handoff package into the archive store. The panel shows recent session archives, recent handoff package archives, and a handoff archive summary that highlights whether the latest package archive is share-ready, which archive is latest, and what action remains before sharing; operators can copy or download that summary as Markdown evidence directly from the panel.
