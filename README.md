...
- External exposure readiness, archive, handoff, session, closeout, closeout archive, and operator handoff checklist gates that check safety controls before a temporary public URL is shared, preserve local evidence, track the active public URL lifetime, confirm the exposure was closed, freeze the shutdown proof, and tell the operator whether the next live `/agent fix` step is safe after closeout.
...
- Demo evidence bundle that combines readiness, smoke-check, configuration, adapter, queue, webhook setup, latest and recent delivery trail, rejected-trigger, quarantine, recent Pull Request evidence, latest external exposure closeout archive evidence, handoff share checklist status, final handoff share center download actions, handoff finalization status, latest final handoff report package archive evidence, launch finalization status, latest launch acceptance closeout/certificate archive evidence, final acceptance delivery/finalization evidence, final acceptance completion closeout/archive evidence, and latest task evidence acceptance certificate archive evidence for one demo-ready summary.
...
- Copyable and downloadable demo session report Markdown that exports the full session snapshot, readiness snapshot trend evidence, task evidence acceptance certificate proof, prepared demo launch commands and archived launch outcomes from the current browser, script, checklist, health contract, next actions, and embedded runbook.
- Copyable and downloadable demo handoff package Markdown that wraps the session report with a concise summary, handoff readiness check, webhook delivery evidence, readiness trend evidence, prepared command counts, archived outcome counts, task evidence, task evidence certificate proof, Pull Request evidence, and next actions.
...
- Demo final handoff report package archives that preserve exact final package snapshots, list recent closeout archives, download archived Markdown reports after the live package view changes, and surface the latest archive in the top-level evidence bundle and copied runbook.
...
- Demo launch evidence package that combines self-hosted launch readiness, session snapshot, live task/PR/webhook proof, evaluation coverage, post-demo handoff proof, final handoff report package archive proof, and a read-only side-effect contract into one shareable Markdown artifact.
- Demo launch evidence share center that turns the latest archived launch evidence package, final handoff report package archive proof, and latest delivery receipt into a final share/no-share status, download actions, evidence notes, receipt freshness state, and downloadable Markdown report.
- Demo launch evidence finalization gate that requires a share-ready launch evidence archive plus a fresh delivery receipt before the launch evidence package is treated as accepted final delivery evidence.
- Demo launch acceptance closeout that combines launch readiness, launch evidence package, share-center delivery proof, final handoff report package archive proof, and finalization status into one final accepted/not-accepted operator report.
- Demo launch acceptance closeout archives that preserve the current final closeout and final handoff report package archive proof as local evidence, list recent closeouts, and download archived Markdown reports after refresh or backend restart in database-backed local profiles.
- Demo launch acceptance certificate that turns the latest accepted READY closeout archive, including final handoff report package archive proof, into one final certified/not-certified readout and downloadable Markdown certificate without mutating GitHub or creating tasks.
- Demo launch acceptance certificate archives that preserve the current certified readout and final handoff report package archive proof as durable local evidence, list recent certificates, and download archived Markdown certificate reports after the live read model changes.
- Demo launch evidence delivery receipts that record local proof that an operator delivered the archived launch evidence through an external channel, with recent receipts and downloadable Markdown evidence, without sending anything externally from PatchPilot.
- Demo final acceptance share package archives that preserve the reviewer-facing final acceptance package, list recent accepted package snapshots, and download archived Markdown reports after the live acceptance package view changes.
- Demo final acceptance share delivery receipts and finalization gate that record local proof of external reviewer-package delivery, require a fresh receipt for the current package archive, and download receipt/finalization Markdown evidence without sending anything externally.
- Demo final acceptance completion archives and a completion evidence bundle that preserve the final READY handoff proof, including package archive, task, delivery receipt, target/channel, freshness, generated time, archived Markdown evidence, and one read-only reviewer-facing bundle after the live finalization view changes.
- Demo final acceptance completion evidence delivery finalization gate that requires the latest completion evidence delivery receipt to match the current completion evidence bundle before the final completion delivery loop is considered closed.
- Demo final acceptance completion closeout and closeout archive that combine accepted demo summary, reviewer-package finalization, completion evidence bundle, completion archive, and completion evidence delivery finalization into one closed/not-closed report for external review, then freeze the READY closeout as durable local evidence with protected audit proof and repeat the latest frozen archive in the top-level evidence bundle and copied runbook.
- Demo final external-review evidence package that aggregates accepted final demo summary, reviewer-package finalization, completion evidence, completion delivery finalization, completion closeout, and the latest frozen closeout archive into one downloadable reviewer-facing Markdown record, repeated in the top-level demo evidence bundle and copied runbook.
- Demo final external-review evidence package archives that preserve the exact reviewer-facing package as durable local evidence, list recent frozen packages, download archived Markdown reports after live evidence changes, and repeat the latest archive in the top-level demo evidence bundle and copied runbook.
- Demo final external-review package delivery receipts that record local proof of external delivery for the latest frozen package archive, list recent receipts, download receipt Markdown reports, and repeat freshness evidence in the top-level demo evidence bundle and copied runbook.
- Demo final external-review package delivery finalization gate that treats the frozen reviewer-facing package as delivered only when the latest package delivery receipt is fresh for the latest archive, downloads a Markdown finalization report, and repeats the finalization status in the top-level demo evidence bundle and copied runbook.
- Demo final external-review package delivery finalization archives that preserve the READY delivery finalization as durable local evidence, list recent frozen delivery-closure records, download archived Markdown reports, and repeat the latest archived closure proof in the top-level demo evidence bundle and copied runbook.
- Demo final external-review delivery certificate that certifies the latest archived delivery-closure record, downloads a Markdown certificate, and gives the final reviewer-facing proof a single read-only operator card.
- Demo final external-review delivery certificate archives that preserve the certified terminal delivery proof as durable local evidence, list recent certificate archives, and download archived Markdown reports after live certificate inputs change.
- Demo final external-review release bundle that turns the latest certified certificate archive into one read-only reviewer handoff bundle with required attachments, checks, evidence notes, and a downloadable Markdown report, repeated in the top-level demo evidence bundle and copied runbook.
- Demo final external-review release bundle delivery receipts and finalization gate that record local proof of sending the latest frozen release-bundle archive, require a fresh receipt before the terminal handoff is treated as delivered, download receipt/finalization Markdown evidence, and repeat the finalization state in the top-level demo evidence bundle and copied runbook.
- Demo final external-review release bundle delivery finalization archives that freeze the READY terminal release-bundle delivery proof, list recent immutable handoff records, download archived Markdown reports, and repeat the latest archive in the top-level demo evidence bundle.
- Demo final external-review release bundle delivery certificate that certifies the latest frozen terminal release-bundle delivery finalization archive, downloads one Markdown certificate, and gives the final reviewer handoff proof a read-only dashboard card.
- Demo final external-review release bundle delivery certificate archives that preserve the certified terminal release-bundle delivery certificate as durable local evidence, list recent frozen certificates, download archived Markdown reports after live certificate inputs change, and repeat the latest terminal certificate archive in the top-level evidence bundle and copied runbook.
- Demo final reviewer handoff package that turns the latest archived terminal release-bundle delivery certificate into one final read-only reviewer package with readiness checks, required attachments, download actions, top-level evidence-bundle visibility, runbook output, and a downloadable Markdown report.
...
- Task evidence archive review and share center that list recent archived task reports, summarize archive counts, identify the newest completed Pull Request-backed archive, show share/no-share readiness, and download a portable Markdown share report without mutating GitHub.
- Task evidence delivery receipts and finalization gate that record local proof of external delivery for the current shareable task archive, require a fresh receipt before final acceptance, and download receipt/finalization Markdown evidence without sending anything externally.
- Task evidence acceptance closeout archives that preserve the finalized task evidence acceptance record as durable local evidence, list recent closeouts, and download archived Markdown reports after refresh or backend restart in database-backed local profiles.
- Task evidence acceptance certificate and certificate archives that turn the latest accepted closeout archive into one certified/not-certified readout, downloadable Markdown certificate, durable local certificate history for external review, and top-level demo evidence bundle proof.
...
After closing and archiving a temporary exposure, use the operator handoff checklist before posting the next live trigger:
...
Before a live `/agent fix` run, inspect the non-mutating publish permission diagnostic:

```bash
curl -H "X-PatchPilot-Admin-Token: $PATCHPILOT_ADMIN_TOKEN" \
  "http://127.0.0.1:8080/api/github/publish-permission-readiness?owner=bingqin2&repository=PatchPilot"
```
...
For the final go/no-go check, run the live launch gate:

```bash
curl -X POST http://127.0.0.1:8080/api/demo/live-launch-gate \
  -H "X-PatchPilot-Admin-Token: $PATCHPILOT_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "repositoryOwner": "bingqin2",
...
Use `/detail` for dashboard-style task inspection. It returns the task audit summary, latest queue item, queue history, accepted-trigger intent audit, persisted pre-execution safety snapshot, latest generated diff, failed-task diagnosis, timeline events, test runs, tool calls, and model calls in one response. Use `/report` to copy a Markdown diagnostic summary for a task, including accepted-trigger intent, pre-execution safety evidence, failure diagnosis, and generated diff when captured. Use `/api/tasks/evidence-packages`, `/api/tasks/evidence-packages/summary`, and `/api/tasks/evidence-packages/share-center` to review recent archived task evidence across tasks, aggregate counts, and determine whether a completed Pull Request-backed archive is ready to share. `GET /api/tasks/evidence-packages/share-center/report/download` downloads the same share-center decision as Markdown. Use `POST /api/tasks/evidence-packages/share-delivery-receipts` to record local proof that an operator delivered the current share-ready task evidence externally; `GET /api/tasks/evidence-packages/share-delivery-receipts` lists recent receipts; and `GET /api/tasks/evidence-packages/share-delivery-receipts/{receiptId}/report/download` downloads a receipt report. `GET /api/tasks/evidence-packages/finalization` is `READY` only when the current shareable archive has a fresh receipt, and `GET /api/tasks/evidence-packages/finalization/report/download` downloads the final acceptance report. The acceptance closeout archive endpoints preserve that finalized acceptance as local evidence: `POST /api/tasks/evidence-packages/acceptance-closeout/archives` stores the current finalization only when it is `READY`; `GET /api/tasks/evidence-packages/acceptance-closeout/archives` lists recent closeouts; and `GET /api/tasks/evidence-packages/acceptance-closeout/archives/{archiveId}/report/download` downloads one archived closeout report. The acceptance certificate endpoints turn the latest accepted closeout archive into final certified evidence: `GET /api/tasks/evidence-packages/acceptance-certificate` returns the current certified/not-certified readout; `GET /api/tasks/evidence-packages/acceptance-certificate/report/download` downloads the current certificate; `POST /api/tasks/evidence-packages/acceptance-certificate/archives` stores the current certified certificate as local evidence; `GET /api/tasks/evidence-packages/acceptance-certificate/archives` lists recent certificate archives; and `GET /api/tasks/evidence-packages/acceptance-certificate/archives/{archiveId}/report/download` downloads one archived certificate report. These archive review, share, finalization, closeout, and certificate endpoints are read-only except receipt, closeout archive, and certificate archive creation; they do not create tasks, call the model, run verification commands, mutate Git, push, open Pull Requests, send messages, or write GitHub comments. Receipt, closeout archive, and certificate archive creation write PatchPilot-local evidence only. The narrower endpoints remain available for focused debugging.
...
curl "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/demo/launch-evidence-package/archives
curl -X POST "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/demo/launch-evidence-package/archives
curl -OJ "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/demo/launch-evidence-package/archives/{archiveId}/report/download
curl "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/demo/launch-evidence-share-center
curl -OJ "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/demo/launch-evidence-share-center/report/download
...
```bash
curl "${ADMIN_HEADER[@]}" "http://127.0.0.1:8080/api/github/publish-readiness?owner=bingqin2&repository=PatchPilot"
```

This endpoint aggregates GitHub credential readiness and repository access readiness into a final `READY`, `NEEDS_ATTENTION`, or `BLOCKED` publish-path result for PatchPilot branch pushes and Pull Request creation. If `owner` and `repository` are omitted, it falls back to `PATCHPILOT_DEMO_REPOSITORY_OWNER` and `PATCHPILOT_DEMO_REPOSITORY_NAME`. It returns non-sensitive checks, a safe publish command shape, evidence notes, and the next operator action. It is read-only: it does not run `git push`, create branches, open Pull Requests, write issue comments, or expose tokens.
...
The evidence bundle endpoint is the one-call demo summary. It includes readiness, smoke checklist, non-sensitive configuration, adapter fixture counts, full evaluation run readiness, queue summary, recent task and Pull Request evidence, combined webhook setup readiness, latest webhook delivery, a capped recent webhook delivery trail with task/rejected-trigger/error outcomes, rejected-trigger counts, active quarantine count, latest handoff share checklist status, final handoff share center status, share-center download actions, latest handoff share delivery receipt summary and freshness state, final handoff finalization status, latest final handoff report package archive status, final launch evidence share-center status, final launch evidence finalization status, latest launch acceptance closeout and certificate archive status, latest final acceptance completion closeout archive status, latest task evidence acceptance certificate archive status, generated time, and next actions. The overall evidence-bundle status is not `READY` until the full evaluation archive evidence, handoff finalization gate, final handoff report package archive evidence, launch evidence finalization gate, accepted launch acceptance closeout archive evidence, certified launch acceptance certificate archive evidence, closed final acceptance completion closeout archive evidence, and certified task evidence acceptance certificate archive evidence are `READY`.
The self-hosted launch readiness endpoint is the final pre-launch readout for local demos. It composes demo readiness, the top-level evidence bundle, handoff finalization, GitHub publish path readiness, GitHub publish permission readiness, credential, webhook setup, and queue/worker signals into one `READY`, `NEEDS_ATTENTION`, or `BLOCKED` status, plus concrete next actions and a downloadable Markdown report. It blocks or warns before launch when the token cannot read the demo repository, publish PatchPilot branches, create Pull Requests, or likely write issue feedback. It is read-only and does not create tasks, call the model, run tests, mutate Git, archive records, send messages, or write to GitHub.
The live launch gate endpoint is the final operator check before posting a real GitHub issue comment. `POST /api/demo/live-launch-gate` accepts the exact repository, issue, trigger user, and `/agent fix` comment, then composes self-hosted launch readiness, webhook setup readiness, live GitHub publish preflight, and live trigger dry-run evidence into one `READY`, `NEEDS_ATTENTION`, or `BLOCKED` result. It returns checks, next actions, the trigger issue URL, a side-effect contract, and a copyable Markdown report. It is read-only and does not create tasks, enqueue work, record rate-limit usage, mutate Git, create branches, open Pull Requests, write GitHub comments, archive records, or expose tokens.
...
The demo launch evidence package endpoint is the final shareable demo artifact. It composes the current self-hosted launch readiness package and demo session snapshot, then returns pre-launch checks, live-run task/Pull Request/webhook proof, evaluation run coverage, post-demo handoff proof, final handoff report package archive readiness, next actions, and a side-effect contract. `GET /api/demo/launch-evidence-package/report/download` formats the same current package as Markdown. Both endpoints are read-only and do not create tasks, call the model, run tests, archive records, mutate Git, send messages, or write to GitHub. The launch evidence package archive endpoints preserve that final package as point-in-time local evidence. `POST /api/demo/launch-evidence-package/archives` captures the current package status, summary, session id, final handoff report package archive proof, key live-run identifiers, timestamp, and Markdown report; `GET /api/demo/launch-evidence-package/archives` lists the latest 20 archives; and `GET /api/demo/launch-evidence-package/archives/{archiveId}/report/download` downloads the exact archived report. The launch evidence share center reads the latest archive and latest delivery receipt, then exposes `GET /api/demo/launch-evidence-share-center` plus `GET /api/demo/launch-evidence-share-center/report/download` so operators can see final share/no-share status, archive count, latest task/Pull Request/webhook/evaluation evidence, final handoff report package archive status and download action, evidence notes, receipt-recorded state, receipt freshness, and a portable Markdown report. `GET /api/demo/launch-evidence-finalization` converts that share center into a final acceptance gate that is `READY` only when the latest archive is share-ready and the latest delivery receipt is fresh for the current archive/session; `GET /api/demo/launch-evidence-finalization/report/download` downloads the final acceptance report as `patchpilot-demo-launch-evidence-finalization.md`. `POST /api/demo/launch-evidence-share-delivery-receipts` records local proof that an operator delivered the archived launch evidence externally, while `GET /api/demo/launch-evidence-share-delivery-receipts` lists recent receipts and `GET /api/demo/launch-evidence-share-delivery-receipts/{receiptId}/report/download` downloads one receipt report. The default profile keeps archives and delivery receipts in memory; `local`, `docker`, and `idea` profiles persist them in MySQL through Flyway/MyBatis. Archive and receipt creation record protected admin audit evidence and remain PatchPilot-local state only. The share-center and finalization endpoints are read-only and do not create tasks, call the model, run tests, archive records, mutate Git, send messages, or write to GitHub. Receipt creation is a local evidence write only: it does not send messages, create tasks, call the model, run tests, mutate Git, or write to GitHub.
The launch acceptance closeout endpoint is the final launch evidence readout. `GET /api/demo/launch-acceptance-closeout` composes self-hosted launch readiness, the live launch evidence package, the latest launch share center, and launch evidence finalization into one `READY`, `NEEDS_ATTENTION`, or `BLOCKED` result with an accepted flag, key evidence identifiers, final handoff report package archive proof, checks, download actions, and Markdown report. `GET /api/demo/launch-acceptance-closeout/report/download` downloads the same report as `patchpilot-launch-acceptance-closeout.md`. Both endpoints are read-only and do not create tasks, call the model, run tests, archive records, mutate Git, send messages, record receipts, or write to GitHub. The closeout archive endpoints preserve final closeout decisions as local evidence: `POST /api/demo/launch-acceptance-closeout/archives` captures the current closeout status, accepted flag, key evidence identifiers, final handoff report package archive proof, timestamp, and Markdown report; `GET /api/demo/launch-acceptance-closeout/archives` lists the latest 20 archives; and `GET /api/demo/launch-acceptance-closeout/archives/{archiveId}/report/download` downloads one archived report. The top-level evidence bundle and copied runbook repeat the latest closeout archive status, accepted flag, linked launch evidence archive, delivery receipt, and download actions so operators can confirm final acceptance evidence without opening the launch package panel. Archive creation records protected admin audit evidence and remains PatchPilot-local state only: it does not create tasks, call the model, run tests, mutate Git, send messages, record receipts, or write to GitHub.
The launch acceptance certificate endpoint is the certified external-review record derived from the latest accepted closeout archive. `GET /api/demo/launch-acceptance-certificate` reports certified only when the latest closeout archive is `READY` and accepted, then carries the closeout's final handoff report package archive proof into the current certificate; `GET /api/demo/launch-acceptance-certificate/report/download` downloads the current certificate report. The certificate archive endpoints preserve that decision as local evidence: `POST /api/demo/launch-acceptance-certificate/archives` stores the current certificate status, certified flag, linked closeout archive, linked launch evidence archive, final handoff report package archive proof, delivery receipt, generated time, archived time, download actions, and Markdown report; `GET /api/demo/launch-acceptance-certificate/archives` lists recent archives; and `GET /api/demo/launch-acceptance-certificate/archives/{archiveId}/report/download` downloads one archived certificate report. The top-level evidence bundle and copied runbook repeat the latest certificate archive status, certified flag, linked closeout archive, linked launch evidence archive, delivery receipt, summary, next action, and download actions so operators can confirm the external-review launch record without opening the launch package panel. Certificate generation and archive creation remain PatchPilot-local evidence operations and do not create tasks, call the model, run tests, mutate Git, send messages, record receipts, or write to GitHub.
The final demo acceptance summary endpoint combines the latest archived launch acceptance certificate and latest archived task evidence acceptance certificate into one accepted/not-accepted external-review readout. `GET /api/demo/acceptance-summary` returns the aggregate status, certificate archive identifiers, checks, evidence notes, download actions, side-effect contract, and Markdown report; `GET /api/demo/acceptance-summary/report/download` downloads the same final summary as `patchpilot-final-demo-acceptance-summary.md`. These endpoints are read-only and do not create tasks, call the model, run tests, archive records, mutate Git, send messages, record receipts, or write to GitHub.
The final demo acceptance share package endpoint turns that accepted/not-accepted readout into reviewer-facing send/no-send material. `GET /api/demo/final-acceptance-share-package` returns send-ready status, recommended recipients, required attachments, pre-send checks, a prepared message subject/body, evidence notes, side-effect contract, and Markdown report; `GET /api/demo/final-acceptance-share-package/report/download` downloads the same package as `patchpilot-final-demo-acceptance-share-package.md`. These endpoints are read-only and do not create tasks, call the model, run tests, archive records, mutate Git, send messages, record receipts, or write to GitHub. The final acceptance share package archive endpoints preserve the reviewer-facing package as local evidence after acceptance: `POST /api/demo/final-acceptance-share-package/archives` captures the current package status, send-ready flag, message subject, generated time, archived time, and Markdown report; `GET /api/demo/final-acceptance-share-package/archives` lists recent package archives; and `GET /api/demo/final-acceptance-share-package/archives/{archiveId}/report/download` downloads one archived package report. Archive creation records protected admin audit evidence and remains PatchPilot-local state only: it does not create tasks, call the model, run tests, mutate Git, send messages, record receipts, or write to GitHub.
The final acceptance completion archive endpoints preserve the READY finalization proof after the reviewer-facing package has a fresh delivery receipt. `POST /api/demo/final-acceptance-completion-archives` stores the current finalization only when it is `READY`, including package archive id, task id, delivery receipt id, target/channel, freshness, evidence notes, generated time, archived time, and Markdown report; `GET /api/demo/final-acceptance-completion-archives` lists recent completion archives; and `GET /api/demo/final-acceptance-completion-archives/{archiveId}/report/download` downloads one archived completion report. `GET /api/demo/final-acceptance-completion-evidence-bundle` summarizes the latest finalization and latest completion archive into one reviewer-facing proof bundle with status, share-ready flag, key evidence identifiers, evidence notes, download actions, and a side-effect contract; `GET /api/demo/final-acceptance-completion-evidence-bundle/report/download` downloads the same bundle as Markdown. `GET /api/demo/final-acceptance-completion-evidence-delivery-finalization` compares that current bundle with the latest completion evidence delivery receipt and reports READY, NEEDS_ATTENTION, or BLOCKED freshness checks; `GET /api/demo/final-acceptance-completion-evidence-delivery-finalization/report/download` downloads the same finalization report as Markdown. `GET /api/demo/final-acceptance-completion-closeout` combines the accepted demo summary, reviewer-package finalization, completion evidence bundle, latest completion archive, completion evidence delivery receipt, and delivery finalization into one final closed/not-closed report; `GET /api/demo/final-acceptance-completion-closeout/report/download` downloads that closeout as `patchpilot-final-acceptance-completion-closeout.md`. `POST /api/demo/final-acceptance-completion-closeout/archives` freezes the current closeout only when it is `READY` and closed; `GET /api/demo/final-acceptance-completion-closeout/archives` lists recent frozen closeouts; and `GET /api/demo/final-acceptance-completion-closeout/archives/{archiveId}/report/download` downloads one frozen closeout report. Archive creation records protected admin audit evidence and remains PatchPilot-local state only. The evidence-bundle, completion delivery finalization, and live completion closeout endpoints are read-only. These routes do not send messages, create tasks, call the model, run tests, mutate Git, record receipts, or write to GitHub.
...
The demo handoff package endpoints format a higher-level Markdown artifact for post-demo sharing. `POST /api/demo/handoff-package` returns a copyable package, and `POST /api/demo/handoff-package/download` returns the same package as a Markdown attachment. The package includes a concise handoff summary, a handoff readiness check, readiness trend evidence, recent webhook delivery evidence, recent task and Pull Request evidence, prepared-command and archived-outcome counts, task evidence certificate proof, next actions, prepared launch commands, archived launch outcomes, and the embedded session report. The readiness check reports `READY`, `NEEDS_ATTENTION`, or `BLOCKED` based on demo status, recent task-created webhook delivery evidence, recent completed task evidence, Pull Request evidence, prepared command context, archived outcome evidence, task evidence certificate readiness, and readiness trend baseline. `GET /api/demo/handoff-readiness` and `POST /api/demo/handoff-readiness` expose the same readiness status, summary, overall next action, and check rows as structured JSON; each check row includes its own next action. The dashboard uses the POST path with browser-local prepared commands and archived outcomes so the visible readiness preview matches the generated handoff package and tells the operator exactly what to fix before sharing. A recent delivery that requires redelivery, such as an invalid signature or backend processing failure, blocks the handoff until the operator fixes the setup and redelivers from GitHub. These endpoints are read-only and do not create tasks, call the model, run tests, mutate Git, archive records, or write to GitHub.
...
curl "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/finalization
curl -OJ "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/finalization/report/download
curl -X POST "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-closeout/archives
curl "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-closeout/archives
curl -OJ "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-closeout/archives/{archiveId}/report/download
curl "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-certificate
curl -OJ "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-certificate/report/download
curl -X POST "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-certificate/archives
curl "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-certificate/archives
curl -OJ "${ADMIN_HEADER[@]}" http://127.0.0.1:8080/api/tasks/evidence-packages/acceptance-certificate/archives/{archiveId}/report/download
```

The task evidence archive review endpoints list recent archived task reports across tasks and summarize the current archive inventory, including completed/failed/pending-review/cancelled counts, latest archive identifiers, a side-effect contract, and the next operator action. The dashboard surfaces the same data in `Task evidence archive review` so operators can download archived task reports or open the related task without selecting each task first. The same panel also shows the task evidence share center, finalization status, finalization checks, recent delivery receipts, a local receipt form, recent acceptance closeout archives, and recent acceptance certificate archives. Closeout archive creation requires the finalization gate to be `READY`; it preserves the accepted evidence record for later audit and report download. Certificate archive creation requires the current certificate to be certified from the latest accepted closeout archive; it preserves the final certificate for external review after the live read model changes. The top-level demo evidence bundle and copied runbook repeat the latest task evidence certificate archive status, certified flag, linked closeout archive, linked task evidence archive, delivery receipt, task id, Pull Request, summary, next action, and download actions so operators can confirm task-level review proof without opening the task evidence review panel. These review, finalization, closeout, and certificate endpoints are read-only; receipt, closeout archive, and certificate archive creation are local evidence writes only. None of these endpoints create tasks, call the model, run verification commands, mutate Git, push branches, open Pull Requests, send messages, or write GitHub comments.
...
The operator setup checklist stays near the top of the page and reuses already loaded dashboard data to show `Backend connectivity`, `Required credentials`, `GitHub credentials`, `GitHub publish readiness`, `Webhook public URL`, `Repository access`, `Safety policy`, `Demo target policy`, `Repository preflight scope`, `Model provider health`, `Adapter fixtures`, `Adapter runtimes`, `Queue health`, `Worker heartbeat`, and `Recent PR evidence` before a live `/agent fix` demo. It is read-only and does not create tasks or mutate configuration.
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, full evaluation run readiness, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, latest handoff share checklist status, final handoff share center evidence, latest handoff share delivery receipt evidence, handoff finalization status, final launch evidence share-center status, final launch evidence finalization status, latest launch acceptance closeout archive evidence, latest launch acceptance certificate archive evidence, final external-review package/archive/delivery/finalization/archive/release-bundle/release-bundle-archive/release-bundle-delivery-finalization-archive/release-bundle-delivery-certificate-archive/final-reviewer-handoff-package evidence, and latest task evidence acceptance certificate archive evidence into one status, count set, and next-action list. Its evidence cards show whether the latest full evaluation archive is ready, which languages/build systems and safety rejection categories are covered, whether the GitHub payload URL, secret-backed setup, post-demo share evidence, archived launch evidence, accepted launch finalization evidence, final closeout archive evidence, launch certificate archive evidence, final external-review package delivery finalization, archived final external-review package delivery finalization, final external-review release bundle, archived final external-review release bundle, archived final external-review release bundle delivery finalization, archived final external-review release bundle delivery certificate, final reviewer handoff package, and task certificate archive evidence are ready, which final handoff, launch, external-review, and task-level evidence artifacts should be downloaded, whether a delivery receipt is missing, fresh for the current archive/session, or stale from an older package, and whether the handoff, launch, external-review, and task evidence are finalized before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff, including launch evidence share-center, finalization, closeout archive acceptance status, launch certificate archive certification status, final external-review package delivery finalization status, archived final external-review package delivery closure status, final external-review release bundle readiness, final external-review release bundle archive readiness, final external-review release bundle delivery certificate status, final release-bundle delivery certificate archive status, final reviewer handoff package readiness, and task evidence certificate archive certification status.
The final demo acceptance panel is backed by `GET /api/demo/acceptance-summary`, `GET /api/demo/acceptance-summary/report/download`, `GET /api/demo/final-acceptance-share-package`, `GET /api/demo/final-acceptance-share-package/report/download`, and the final acceptance archive, receipt, finalization, completion archive, completion evidence bundle, completion evidence delivery receipt, completion delivery finalization, completion closeout, and completion closeout archive endpoints. It gives the operator one final accepted/not-accepted answer across launch and task certificates, shows the linked archive ids and checks, downloads a portable Markdown summary, presents a reviewer-facing share package with recipients, attachments, pre-send checks, prepared subject/body text, copy action, Markdown download, local delivery receipt recording, finalization proof, final completion archive history, one downloadable final completion evidence bundle, a local form/history for recording where that final completion evidence bundle was delivered, a read-only finalization gate proving whether the latest receipt closes the current completion bundle, one final closeout report proving whether the external-review completion loop is closed, recent frozen closeout reports, a final external-review package archive/receipt/finalization workflow, recent archived final external-review delivery closure reports, a final delivery certificate with archive history for durable review evidence, and a final external-review release bundle with archive history for frozen terminal reviewer handoff proof. Completion evidence delivery receipts are created through `POST /api/demo/final-acceptance-completion-evidence-delivery-receipts`, listed through `GET /api/demo/final-acceptance-completion-evidence-delivery-receipts`, and downloaded through `GET /api/demo/final-acceptance-completion-evidence-delivery-receipts/{receiptId}/report/download`; completion delivery finalization is loaded through `GET /api/demo/final-acceptance-completion-evidence-delivery-finalization` and downloaded through `GET /api/demo/final-acceptance-completion-evidence-delivery-finalization/report/download`; completion closeout is loaded through `GET /api/demo/final-acceptance-completion-closeout`, downloaded through `GET /api/demo/final-acceptance-completion-closeout/report/download`, archived through `POST /api/demo/final-acceptance-completion-closeout/archives`, listed through `GET /api/demo/final-acceptance-completion-closeout/archives`, and downloaded from archive history through `GET /api/demo/final-acceptance-completion-closeout/archives/{archiveId}/report/download`. Final external-review delivery finalization archives are created through `POST /api/demo/final-external-review-evidence-package/delivery-finalization/archives`, listed through `GET /api/demo/final-external-review-evidence-package/delivery-finalization/archives`, and downloaded through `GET /api/demo/final-external-review-evidence-package/delivery-finalization/archives/{archiveId}/report/download`; the final delivery certificate is loaded through `GET /api/demo/final-external-review-delivery-certificate`, downloaded through `GET /api/demo/final-external-review-delivery-certificate/report/download`, archived through `POST /api/demo/final-external-review-delivery-certificate/archives`, listed through `GET /api/demo/final-external-review-delivery-certificate/archives`, and downloaded from archive history through `GET /api/demo/final-external-review-delivery-certificate/archives/{archiveId}/report/download`; the final release bundle is loaded through `GET /api/demo/final-external-review-release-bundle`, downloaded through `GET /api/demo/final-external-review-release-bundle/report/download`, archived through `POST /api/demo/final-external-review-release-bundle/archives`, listed through `GET /api/demo/final-external-review-release-bundle/archives`, and downloaded from archive history through `GET /api/demo/final-external-review-release-bundle/archives/{archiveId}/report/download`. The share package, completion archive, completion evidence receipt, closeout archive, final external-review package archive, final external-review delivery receipt, final external-review delivery finalization archive, final delivery certificate archive, and final release-bundle archive actions are operator aids; PatchPilot does not send external messages from this panel.
Final release-bundle delivery finalization archives are created through `POST /api/demo/final-external-review-release-bundle/delivery-finalization/archives`, listed through `GET /api/demo/final-external-review-release-bundle/delivery-finalization/archives`, and downloaded through `GET /api/demo/final-external-review-release-bundle/delivery-finalization/archives/{archiveId}/report/download`. These archives freeze the terminal READY release-bundle delivery proof as local evidence and do not send external messages.
The demo session snapshot combines the current evidence bundle, readiness snapshot trend, browser-local prepared launch commands, browser-local archived launch outcomes, script, runbook, operator checklist, health contract, share summary, and next actions in one panel so an operator can confirm both the current demo state and readiness movement before or after the live `/agent fix` trigger. Its `Copy session report` and `Download session report` actions fetch Markdown reports on demand with the current readiness trend, prepared command, and outcome archive context for review notes, issue comments, or handoff messages. Its `Copy handoff package` and `Download handoff package` actions fetch a higher-level Markdown package with summary counts, webhook delivery readiness, readiness trend evidence, task/PR evidence, next actions, prepared commands, archived outcomes, and the embedded session report. Its archive actions capture the current session report or final handoff package into the archive store. The panel shows recent session archives, recent handoff package archives, a handoff archive summary that highlights whether the latest package archive is share-ready, a final handoff share center with send/no-send status, task certificate gate, and download actions, a handoff finalization gate with acceptance status, task evidence certificate check, and a Markdown report download, share instructions with recipients, attachments, checks, and a message template, a local delivery receipt form/list with Markdown receipt downloads, and a handoff share checklist that turns that summary into concrete share/no-share checks with copyable/downloadable Markdown evidence.
