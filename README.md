...
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, and the latest handoff share checklist status into one status, count set, and next-action list. Its evidence cards show whether the GitHub payload URL, secret-backed setup, and post-demo share evidence are ready before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff.
