...
These endpoints return benchmark-ready cases for supported issue-to-PR fixes and safety rejections, a readiness summary, and a benchmark-shaped preview report with expected verification commands, known gaps, side-effect contract, and next action. The read-only catalog, summary, and preview endpoints do not create tasks, call the model, clone repositories, run tests, mutate Git, or write to GitHub. The snapshot archive endpoint stores the current preview as PatchPilot-local evidence only; it still does not call the model, clone repositories, run verification commands, mutate Git, or write to GitHub. The dashboard shows the same catalog, readiness summary, run preview, and recent snapshot archives in `Evaluation case catalog` and can copy or download Markdown reports for demo handoff or review notes.
