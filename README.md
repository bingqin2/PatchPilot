...
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, full evaluation run readiness, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, latest handoff share checklist status, final handoff share center evidence, latest handoff share delivery receipt evidence, handoff finalization status, and final launch evidence share-center status into one status, count set, and next-action list. Its evidence cards show whether the latest full evaluation archive is ready, which languages/build systems and safety rejection categories are covered, whether the GitHub payload URL, secret-backed setup, post-demo share evidence, and archived launch evidence are ready, which final handoff and launch evidence artifacts should be downloaded, whether a delivery receipt is missing, fresh for the current archive/session, or stale from an older package, and whether the handoff is finalized before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff.
