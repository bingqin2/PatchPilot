...
The final demo acceptance panel is backed by `GET /api/demo/acceptance-summary` and `GET /api/demo/acceptance-summary/report/download`. It gives the operator one final accepted/not-accepted answer across launch and task certificates, shows the linked archive ids and checks, and downloads a portable Markdown summary for external review.
