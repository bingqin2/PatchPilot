...
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, full evaluation run readiness, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, latest handoff share checklist status, final handoff share center evidence, latest handoff share delivery receipt evidence, handoff finalization status, final launch evidence share-center status, final launch evidence finalization status, latest launch acceptance closeout archive evidence, latest launch acceptance certificate archive evidence, and latest task evidence acceptance certificate archive evidence into one status, count set, and next-action list. Its evidence cards show whether the latest full evaluation archive is ready, which languages/build systems and safety rejection categories are covered, whether the GitHub payload URL, secret-backed setup, post-demo share evidence, archived launch evidence, accepted launch finalization evidence, final closeout archive evidence, launch certificate archive evidence, and task certificate archive evidence are ready, which final handoff, launch, and task-level evidence artifacts should be downloaded, whether a delivery receipt is missing, fresh for the current archive/session, or stale from an older package, and whether the handoff, launch, and task evidence are finalized before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff, including launch evidence share-center, finalization, closeout archive acceptance status, launch certificate archive certification status, and task evidence certificate archive certification status.
