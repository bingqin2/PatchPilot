...
The operator setup checklist stays near the top of the page and reuses already loaded dashboard data to show `Backend connectivity`, `Required credentials`, `Safety policy`, `Model provider health`, `Adapter fixtures`, `Adapter runtimes`, `Queue health`, `Worker heartbeat`, and `Recent PR evidence` before a live `/agent fix` demo. It is read-only and does not create tasks or mutate configuration.
