...
- Demo final acceptance completion archives that preserve the final READY handoff proof, including package archive, task, delivery receipt, target/channel, freshness, generated time, and archived Markdown evidence after the live finalization view changes.
...
The final acceptance completion archive endpoints preserve the READY finalization proof after the reviewer-facing package has a fresh delivery receipt. `POST /api/demo/final-acceptance-completion-archives` stores the current finalization only when it is `READY`, including package archive id, task id, delivery receipt id, target/channel, freshness, evidence notes, generated time, archived time, and Markdown report; `GET /api/demo/final-acceptance-completion-archives` lists recent completion archives; and `GET /api/demo/final-acceptance-completion-archives/{archiveId}/report/download` downloads one archived completion report. Archive creation records protected admin audit evidence and remains PatchPilot-local state only: it does not send messages, create tasks, call the model, run tests, mutate Git, record receipts, or write to GitHub.
...
The final demo acceptance panel is backed by `GET /api/demo/acceptance-summary`, `GET /api/demo/acceptance-summary/report/download`, `GET /api/demo/final-acceptance-share-package`, `GET /api/demo/final-acceptance-share-package/report/download`, and the final acceptance archive, receipt, finalization, and completion archive endpoints. It gives the operator one final accepted/not-accepted answer across launch and task certificates, shows the linked archive ids and checks, downloads a portable Markdown summary, presents a reviewer-facing share package with recipients, attachments, pre-send checks, prepared subject/body text, copy action, Markdown download, local delivery receipt recording, finalization proof, and final completion archive history. The share package and completion archive actions are operator aids; PatchPilot does not send external messages from this panel.
