...
- Copyable and downloadable demo handoff package Markdown that wraps the session report with a concise summary, handoff readiness check, readiness trend evidence, prepared command counts, archived outcome counts, task evidence, Pull Request evidence, and next actions.
...
- Demo handoff package archive that stores the latest final handoff packages separately from session reports, including dashboard-supplied command and outcome context, with copy/download access after a live run.
...
The demo handoff package endpoints format a higher-level Markdown artifact for post-demo sharing. `POST /api/demo/handoff-package` returns a copyable package, and `POST /api/demo/handoff-package/download` returns the same package as a Markdown attachment. The package includes a concise handoff summary, a handoff readiness check, readiness trend evidence, recent task and Pull Request evidence, prepared-command and archived-outcome counts, next actions, prepared launch commands, archived launch outcomes, and the embedded session report. The readiness check reports `READY`, `NEEDS_ATTENTION`, or `BLOCKED` based on demo status, recent completed task evidence, Pull Request evidence, prepared command context, archived outcome evidence, and readiness trend baseline. These endpoints are read-only and do not create tasks, call the model, run tests, mutate Git, archive records, or write to GitHub.
...
The demo handoff package archive endpoints store the final handoff package separately from session-report archives. `POST /api/demo/handoff-package-archives` captures the current snapshot and generated handoff package, `GET /api/demo/handoff-package-archives` lists the latest 20 package archives, and `GET /api/demo/handoff-package-archives/{archiveId}/report/download` downloads one archived package. The default profile keeps package archives in memory; `local`, `docker`, and `idea` profiles persist them in MySQL. Archive creation records a protected admin audit row and remains PatchPilot-local state only.
