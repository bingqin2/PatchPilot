...
- Demo readiness gate that summarizes credentials, model provider health, adapter fixtures, adapter runtime executables, queue health, worker heartbeat readiness, and recent PR evidence before a live smoke run.
...
- Copyable and downloadable demo session report Markdown that exports the full session snapshot, prepared demo launch commands and archived launch outcomes from the current browser, script, checklist, health contract, next actions, and embedded runbook.
- Copyable and downloadable demo handoff package Markdown that wraps the session report with a concise summary, prepared command counts, archived outcome counts, task evidence, Pull Request evidence, and next actions.
...
- Read-only evaluation case catalog, readiness summary, fixture-readiness report, run preview report, and local snapshot archive for Java/Maven, Node/npm, Python/pytest, Go, unsafe-trigger rejection, and vague-trigger rejection scenarios, with expected commands, expected files, fixture drift checks, success criteria, safety expectations, known gaps, next action, and copyable/downloadable Markdown evidence.
...
These endpoints return benchmark-ready cases for supported issue-to-PR fixes and safety rejections, a readiness summary, a fixture-readiness check, and a benchmark-shaped preview report with expected verification commands, known gaps, side-effect contract, and next action. The fixture-readiness endpoint checks that supported cases still have checked-in fixture directories, detect as the expected language adapters, and contain the expected changed files; safety-rejection cases are reported as no-fixture-required. The read-only catalog, summary, fixture-readiness, and preview endpoints do not create tasks, call the model, clone repositories, run tests, mutate Git, or write to GitHub. The snapshot archive endpoint stores the current preview as PatchPilot-local evidence only; it still does not call the model, clone repositories, run verification commands, mutate Git, or write to GitHub. The dashboard shows the same catalog, readiness summary, fixture readiness, run preview, and recent snapshot archives in `Evaluation case catalog` and can copy or download Markdown reports for demo handoff or review notes.
...
The operator setup checklist stays near the top of the page and reuses already loaded dashboard data to show `Backend connectivity`, `Required credentials`, `GitHub credentials`, `Repository access`, `Safety policy`, `Demo target policy`, `Repository preflight scope`, `Model provider health`, `Adapter fixtures`, `Adapter runtimes`, `Queue health`, `Worker heartbeat`, and `Recent PR evidence` before a live `/agent fix` demo. It is read-only and does not create tasks or mutate configuration.
...
The demo session snapshot combines the current evidence bundle, browser-local prepared launch commands, browser-local archived launch outcomes, script, runbook, operator checklist, health contract, share summary, and next actions in one panel so an operator can confirm the demo state before or after the live `/agent fix` trigger. Its `Copy session report` and `Download session report` actions fetch Markdown reports on demand with the current prepared command and outcome archive context for review notes, issue comments, or handoff messages. Its `Copy handoff package` and `Download handoff package` actions fetch a higher-level Markdown package with summary counts, task/PR evidence, next actions, prepared commands, archived outcomes, and the embedded session report. Its `Archive session` action captures the current report into the archive store and lists recent archived reports with their own copy/download actions.
