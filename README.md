...
- Demo evidence bundle that combines readiness, smoke-check, configuration, adapter, queue, webhook setup, latest and recent delivery trail, rejected-trigger, quarantine, recent Pull Request evidence, handoff share checklist status, and final handoff share center download actions for one demo-ready summary.
...
- Demo handoff share center that combines the latest package archive summary, share checklist, and latest delivery receipt into one final send/no-send status, download action list, evidence notes, and downloadable Markdown report.
...
The evidence bundle endpoint is the one-call demo summary. It includes readiness, smoke checklist, non-sensitive configuration, adapter fixture counts, queue summary, recent task and Pull Request evidence, combined webhook setup readiness, latest webhook delivery, a capped recent webhook delivery trail with task/rejected-trigger/error outcomes, rejected-trigger counts, active quarantine count, latest handoff share checklist status, final handoff share center status, share-center download actions, latest handoff share delivery receipt summary, generated time, and next actions.
...
The demo evidence bundle is the top-level demo readout. It combines readiness, smoke-check, configuration, adapter fixture, queue, webhook setup readiness, latest delivery, rejected-trigger, quarantine, recent Pull Request signals, latest handoff share checklist status, final handoff share center evidence, and latest handoff share delivery receipt evidence into one status, count set, and next-action list. Its evidence cards show whether the GitHub payload URL, secret-backed setup, and post-demo share evidence are ready, including which final handoff artifacts should be downloaded and whether a delivery receipt has been recorded, before the operator relies on delivery history. Its `Copy runbook` action copies a Markdown report suitable for demo notes or operator handoff.
The demo session snapshot combines the current evidence bundle, readiness snapshot trend, browser-local prepared launch commands, browser-local archived launch outcomes, script, runbook, operator checklist, health contract, share summary, and next actions in one panel so an operator can confirm both the current demo state and readiness movement before or after the live `/agent fix` trigger. Its `Copy session report` and `Download session report` actions fetch Markdown reports on demand with the current readiness trend, prepared command, and outcome archive context for review notes, issue comments, or handoff messages. Its `Copy handoff package` and `Download handoff package` actions fetch a higher-level Markdown package with summary counts, webhook delivery readiness, readiness trend evidence, task/PR evidence, next actions, prepared commands, archived outcomes, and the embedded session report. Its archive actions capture the current session report or final handoff package into the archive store. The panel shows recent session archives, recent handoff package archives, a handoff archive summary that highlights whether the latest package archive is share-ready, a final handoff share center with send/no-send status and download actions, share instructions with recipients, attachments, checks, and a message template, a local delivery receipt form/list with Markdown receipt downloads, and a handoff share checklist that turns that summary into concrete share/no-share checks with copyable/downloadable Markdown evidence.
