...
- Demo handoff package archive that stores the latest final handoff packages separately from session reports, including dashboard-supplied command and outcome context plus archived readiness status, next action, and check counts, with copy/download access after a live run.
...
The demo handoff package archive endpoints store the final handoff package separately from session-report archives. `POST /api/demo/handoff-package-archives` captures the current snapshot, generated handoff package, handoff readiness status, readiness summary, next action, and ready/warning/blocked check counts; `GET /api/demo/handoff-package-archives` lists the latest 20 package archives, and `GET /api/demo/handoff-package-archives/{archiveId}/report/download` downloads one archived package. The default profile keeps package archives in memory; `local`, `docker`, and `idea` profiles persist them in MySQL. Archive creation records a protected admin audit row and remains PatchPilot-local state only.
